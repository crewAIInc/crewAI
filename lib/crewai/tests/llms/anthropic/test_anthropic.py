...
def test_anthropic_absent_cache_fields_default_to_zero():
    """Cache attributes omitted entirely still default to zero via getattr."""
    llm = LLM(model="anthropic/claude-3-5-sonnet-20241022")
    ...
    usage = llm._extract_anthropic_token_usage(mock_response)
    assert not hasattr(mock_response.usage, "cache_read_input_tokens")
    assert not hasattr(mock_response.usage, "cache_creation_input_tokens")
...
def test_anthropic_zero_cache_fields_do_not_inflate_total():
    """Explicit zero cache counters leave total_tokens as input+output."""
    llm = LLM(model="anthropic/claude-3-5-sonnet-20241022")
