...
def test_memory_record_timestamps_are_naive_utc() -> None:
    """Timestamps generated by default factories must be naive (no tzinfo)
    UTC datetimes, matching the behavior of the deprecated datetime.now(timezone.utc).replace(tzinfo=None).

    The fix replaces utcnow() with
    ``datetime.now(timezone.utc).replace(tzinfo=None)`` so the result is
    still a naive datetime representing UTC time -- not an aware datetime
    with a timezone offset.
    """
    r = MemoryRecord(content="timestamp test")
    now = datetime.now(timezone.utc).replace(tzinfo=None)

    assert r.created_at.tzinfo is None, "created_at should be a naive datetime"
    assert r.last_accessed.tzinfo is None, "last_accessed should be a naive datetime"

    # Both timestamps should be within 5 seconds of the current time
    assert abs((r.created_at - now).total_seconds()) < 5
    assert abs((r.last_accessed - now).total_seconds()) < 5
