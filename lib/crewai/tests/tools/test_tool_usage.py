...
def test_format_result_serializes_nested_dict_as_json():
    """Nested dictionary outputs should become valid JSON strings."""
    ...
    formatted_result = tool_usage._format_result(raw_result)

    assert isinstance(formatted_result, str)
    assert json.loads(formatted_result) == raw_result
    assert formatted_result.startswith('{"status"')

def test_format_result_serializes_nested_list_as_json():
    """Nested list outputs should become valid JSON strings."""
...
def test_format_result_preserves_string_output():
    """Ordinary string outputs should remain unchanged."""
    ...
    raw_result = "Normal tool output"

    formatted_result = tool_usage._format_result(raw_result)

    assert formatted_result == raw_result
