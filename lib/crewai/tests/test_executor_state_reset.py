...
def test_executor_state_reset_on_invoke():
    ...
    mock_llm.stop = []
    mock_llm.call.return_value = "Final Answer: complete output here"
    ...
    assert result["output"] == "complete output here"
    # Old messages must be gone
    old_msgs = [m for m in executor.messages if m.get("content") == "old system prompt"]
    assert len(old_msgs) == 0, "Old messages were not cleared"
    # Iterations must have been reset to 0 and incremented once during this single-pass invoke
    assert executor.iterations == 1, "Iterations counter was not reset to 0 before invoke"
