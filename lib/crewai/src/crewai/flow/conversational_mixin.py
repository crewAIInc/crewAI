"""Conversational graph and helpers for ``Flow``.

This module keeps the conversational extension separate so
``crewai.flow.runtime`` can stay focused on the execution engine.
``crewai.flow.flow`` composes this mixin onto the public ``Flow`` class.
...
Import surface:
    ...
    - The data types this mixin uses live in
      :mod:`crewai.flow.conversational`.
"""
