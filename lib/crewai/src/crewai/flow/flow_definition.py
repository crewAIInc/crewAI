"""
Flow definition: the structural model derived from the DSL.

Condition predicates, condition decoding, AST-based router-path extraction,
graph/level analysis, and ``extract_flow_definition`` (the structural
registries the runtime metaclass attaches to a Flow class). Previously these
lived in ``crewai.flow.utils``, which now re-exports from here.
...
def extract_flow_definition(
    ...
    """Extract the structural flow registries from a class namespace.

    Walks the decorated methods in ``namespace`` and returns the
    ``(start_methods, listeners, routers, router_paths)`` registries that the
    runtime metaclass attaches to a Flow class. This is the structural half of
    what used to live inline in ``FlowMeta.__new__``.
    """
    start_methods = []
    listeners = {}
    router_paths = {}
