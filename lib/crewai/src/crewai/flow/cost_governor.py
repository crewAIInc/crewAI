"""Cost governor decorator for Flow methods.

This module provides the @cost_governor decorator that enables budget and token
limit enforcement within CrewAI Flows. It allows pausing, stopping, or warning
when token usage or estimated costs exceed configured thresholds.

Supports HITL (human-in-the-loop) integration for budget approval when on_exceed='pause'.

Example:
    ```python
    from crewai.flow import Flow, start, listen, cost_governor
...
class BudgetExceededError(Exception):
    """Raised when budget or token limit is exceeded and on_exceed='stop'.
...
class CostGovernorConfig:
    """Configuration for the @cost_governor decorator.

    Attributes:
        budget_limit: Maximum allowed cost in USD. None means no budget limit.
        token_limit: Maximum allowed tokens. None means no token limit.
...
@dataclass
class CostTracker:
    """Tracks cumulative token usage and estimated costs within a Flow.
    ...
    def add_usage(
        ...
        Args:
            usage: The UsageMetrics from a crew execution.
        ...
        if usage.prompt_tokens == 0 and usage.completion_tokens == 0 and usage.total_tokens > 0:
            ...
            prompt_cost_usd = (usage.prompt_tokens / 1_000_000) * input_cost
            completion_cost_usd = (usage.completion_tokens / 1_000_000) * output_cost
    ...
    def effective_budget(self) -> float | None:
        ...
        if self.budget_limit is None:
            return None
        return self.budget_limit + self.approved_budget
...
def _extract_usage_from_result(result: Any) -> tuple[UsageMetrics | None, str | None]:
    ...
    # Handle list of CrewOutputs
    if isinstance(result, list):
        combined = UsageMetrics()
        for item in result:
            if isinstance(item, CrewOutput) and item.token_usage:
                combined.add_usage_metrics(item.token_usage)
    ...
    # Handle objects with token_usage attribute
...
def _request_budget_approval(
    ...
    Args:
        ...
        config: The cost governor configuration.
        tracker: The current cost tracker.
    ...
    budget_str = f"${tracker.budget_limit:.2f}" if tracker.budget_limit else "unlimited"
    ...
    message = (
        ...
        f"has spent {current_str} of {budget_str} budget.\n\n"
        f"Tokens used: {tracker.total_tokens:,}\n"
        f"Requests made: {tracker.successful_requests}\n\n"
        f"To continue, approve additional budget or deny to stop the flow."
    )
...
def _handle_exceeded_budget(
    ...
    """Handle budget/token limit exceeded based on on_exceed setting.
    ...
    if config.on_exceed == "warn":
        logger.warning(
            f"Budget/token limit exceeded in flow '{flow_instance.name or flow_instance.__class__.__name__}': "
            f"cost=${tracker.estimated_cost:.2f}, tokens={tracker.total_tokens}, "
            f"budget_limit=${tracker.budget_limit}, token_limit={tracker.token_limit}"
        )
        return
...
def cost_governor(
    ...
    """Decorator for Flow methods that enforces budget and token limits.

    This decorator wraps a Flow method to:
    1. Execute the method and capture its result
    2. Extract token usage from the result (if it's a CrewOutput or similar)
    3. Accumulate usage in the flow's cost tracker
    4. Check if budget or token limits are exceeded
    5. Take action based on on_exceed setting (pause, stop, or warn)

    When `on_exceed='pause'`, the decorator uses the HITL (human-in-the-loop)
    infrastructure to request budget approval. If approved, the budget is
    ...
    Returns:
        A decorator function that wraps the method with cost governance.
    ...
    Example:
        Basic budget limit with pause on exceed:
        ```python
        @start()
        ...
        ```python
        @start()
        @cost_governor(token_limit=100000, on_exceed='stop')
        ...
        @cost_governor(
            ...
            token_limit=500000,
            on_exceed='pause',
            cost_map={"my-custom-model": (1.00, 2.00)},
        )
    ...
    def decorator(func: F) -> F:
        ...
        def _ensure_cost_tracker(flow_instance: Flow[Any]) -> CostTracker:
            """Ensure the flow has a cost tracker initialized."""
            if not hasattr(flow_instance, "_cost_tracker") or flow_instance._cost_tracker is None:
                merged_costs = DEFAULT_MODEL_COSTS.copy()
                if config.cost_map:
                    merged_costs.update(config.cost_map)
                flow_instance._cost_tracker = CostTracker(
                ...
                tracker = flow_instance._cost_tracker
                if tracker.budget_limit is None and config.budget_limit is not None:
                    tracker.budget_limit = config.budget_limit
                if tracker.token_limit is None and config.token_limit is not None:
                    tracker.token_limit = config.token_limit
            return flow_instance._cost_tracker
        ...
        def _process_result(
            ...
            tracker = _ensure_cost_tracker(flow_instance)
            ...
            # Check limits
            if tracker.is_budget_exceeded or tracker.is_token_limit_exceeded:
        ...
        # Add cost governor specific attributes
        wrapper.__cost_governor_config__ = config
