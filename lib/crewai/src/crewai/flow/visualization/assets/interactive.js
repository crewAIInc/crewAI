...
class NodeRenderer {
  ...
  render({ ctx, id, x, y }) {
    ...
    return {
      drawNode: () => {
        ...
        this.drawNodeText(ctx, x, y, scale, nodeStyle, opacity);
        ctx.restore();
      },
  ...
  drawNodeShape(ctx, x, y, width, height, scale, nodeStyle, opacity = 1.0, node = null) {
    ...
    if (isDarkMode) {
      // In dark mode, use the theme color
      nodeColor = this.resolveCSSVariable(nodeStyle.bgColor);
    } else {
      // In light mode, use white for all nodes
    ...
    // For inactive nodes, desaturate and darken the color by 40%
    const isInactive = opacity < 0.9;
    if (isInactive) {
      // Convert to HSL to adjust saturation and lightness
      const max = Math.max(r, g, b) / 255;
      const min = Math.min(r, g, b) / 255;
    ...
    // Use full opacity for active nodes, semi-transparent for inactive to create frosty effect
    const glassOpacity = isInactive ? 0.6 : 1.0;
    ...
    const finalBorderColor = this.desaturateColor(borderColor, opacity);
  ...
  drawNodeText(ctx, x, y, scale, nodeStyle, opacity = 1.0) {
    const isInactive = opacity < 0.9;
    ...
    // For inactive nodes, reduce label background color brightness by 95%
    const isDarkMode = document.documentElement.getAttribute('data-theme') === 'dark';
    let labelBgR = 255, labelBgG = 255, labelBgB = 255;
    let labelBgOpacity = 0.2 * opacity;

    if (isInactive) {
      labelBgR = labelBgG = labelBgB = Math.floor(255 * 0.05);
      // Use higher opacity for inactive nodes so the dark color is visible
      labelBgOpacity = 0.5;
    } else if (!isDarkMode) {
      // In light mode, use gray for active node labels
    ...
    // In light mode, use dark text; in dark mode, use theme color
    let fontColor;
    if (isDarkMode) {
  ...
  animateNodeOpacity() {
    ...
    const animate = () => {
      ...
      allNodesList.forEach((node) => {
        ...
        // Make inactive nodes fade more into the backdrop
        const inactiveOpacity = isDarkMode ? 0.75 : 0.55;
    ...
    const animate = () => {
      ...
      edgeIds.forEach((edgeId) => {
        ...
        if (this.highlightedEdges.includes(edge.id)) {
          ...
          // In light mode, use higher opacity for inactive edges for better visibility
          const isDarkMode = document.documentElement.getAttribute('data-theme') === 'dark';
          const targetOpacity = isDarkMode ? 0.25 : 0.45;
          ...
          const isAndOrRouter = edge.dashes || edge.label === "AND";
          const inactiveEdgeColor = isAndOrRouter
            ? "{{ CREWAI_ORANGE }}"
            : getComputedStyle(document.documentElement).getPropertyValue('--edge-or-color').trim();
          ...
          if (group.items.length === 1) {
            ...
            return `
              <div class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
                <div class="trigger-group-label" style="color: ${groupColor}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${groupBgColor}; padding: 3px 8px; border-radius: 3px; display: inline-block; user-select: none;">
                  ${group.items.length} routes <span style="opacity: 0.5; font-size: 9px;">▼</span>
                </div>
...
class NetworkManager {
    ...
    allEdges.forEach((edge) => {
      ...
      const updateData = {
        ...
        shadow: {
          enabled: true,
          color: "rgba(0,0,0,0.08)",
          size: CONSTANTS.EDGE.DEFAULT_SHADOW_SIZE,
          x: 1,
          y: 1,
        },
        font: {
  ...
  calculateNetworkPosition(isDrawerOpen) {
    ...
    const domCenterX = leftEdge + (availableWidth / 2);
  ...
  fitToAvailableSpace() {
    ...
    const targetPosition = this.calculateNetworkPosition(isDrawerOpen);
