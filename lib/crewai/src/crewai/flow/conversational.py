"""Conversational types and helpers shared by ``Flow``.
...
@dataclass
class RouterConfig:
    """LLM router configuration for conversational ``Flow``.
...
class ConversationConfig:
    """Class-level configuration for conversational ``Flow``.
...
class ConversationState(BaseModel):
    """Structured state for conversational ``Flow``.
