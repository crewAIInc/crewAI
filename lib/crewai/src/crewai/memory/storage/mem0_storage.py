...
class Mem0Storage(Storage):
    ...
    def _parse_config(config):
        ...
        Raises:
            TypeError: If config is not a dict, JSON string, or None.
            ValueError: If config is a string but not valid JSON or does not
                        decode to a dict.
        """
        if config is None:
            return {}
        ...
        if isinstance(config, str):
            ...
            if not isinstance(parsed, dict):
                raise TypeError(
                    f"config JSON must decode to a dict, got {type(parsed).__name__}"
                )
            return parsed
        raise TypeError(
            f"config must be a dict, a JSON string, or None, got {type(config).__name__}"
        )
