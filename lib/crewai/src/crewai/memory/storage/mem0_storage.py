...
class Mem0Storage(Storage):
    ...
    def _parse_config(config):
        """Parse config into a dict, handling JSON strings, dicts, and None.

        This handles the common case where config is loaded from environment
        variables, config files, or CLI arguments as a JSON string rather
        than a dict.

        Args:
            config: Configuration as a dict, JSON string, or None.

        Returns:
            dict: Parsed configuration dictionary.

        Raises:
            TypeError: If config is not a dict, JSON string, or None, or if
                       a JSON string decodes to a non-dict type.
            ValueError: If config is a string but not valid JSON.
        """
        if config is None:
            return {}
        ...
        if isinstance(config, str):
            try:
                ...
                raise ValueError(
                    f"Mem0Storage config string is not valid JSON: {exc}"
                ) from exc
            if not isinstance(parsed, dict):
                raise TypeError(
                    f"Mem0Storage config JSON must decode to a dict, "
                    f"got {type(parsed).__name__}"
                )
            return parsed
        raise TypeError(
            f"Mem0Storage config must be a dict or JSON string, "
