        ...
        if self._client:
            try:
                ...
                if bg_loop is not None and bg_loop.is_running():
                    # Schedule close on the background loop — more reliable than
                    # create_task which can be GC'd before it runs.
    ...
    async def _find_records_by_scope(self, scope_prefix: str) -> list[str]:
        ...
        while True:
            ...
            for key_bytes in keys:
                ...
                # Check if scope matches prefix
                if scope_path.startswith(scope_prefix):
