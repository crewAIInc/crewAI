...
class TavilyResearchTool(BaseTool):
    ...
    @staticmethod
    def _extract_content_from_sse(raw: str) -> list[str]:
        """Parse SSE lines and return content fragments as strings."""
        parts: list[str] = []
    ...
    @staticmethod
    def _consume_stream(stream: Generator[bytes, None, None]) -> str:
        """Consume an SSE stream and extract the report content."""
        content_parts: list[str] = []
        for chunk in stream:
            decoded = chunk.decode("utf-8", errors="replace")
            content_parts.extend(
                TavilyResearchTool._extract_content_from_sse(decoded)
            )
        return "".join(content_parts)

    @staticmethod
    async def _consume_async_stream(stream: AsyncGenerator[bytes, None]) -> str:
        """Consume an async SSE stream and extract the report content."""
        content_parts: list[str] = []
    ...
    def _run(
        ...
        """Synchronously creates Tavily research tasks and returns results."""
        if not self._client:
            raise ValueError(
    ...
    async def _arun(
        ...
        """Asynchronously creates Tavily research tasks and returns results."""
        ...
        if use_stream:
            return await self._consume_async_stream(result)
