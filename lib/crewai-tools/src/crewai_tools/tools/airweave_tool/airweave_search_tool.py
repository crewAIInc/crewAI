...
try:
    from airweave import AsyncAirweaveSDK, AirweaveSDK
...
class AirweaveSearchToolSchema(BaseModel):
    ...
    query: str = Field(..., description="The search query text to find relevant documents and data.")
    collection_id: str = Field(..., description="The unique readable identifier of the Airweave collection to search (e.g., 'finance-data-2024').")
    limit: Optional[int] = Field(default=10, description="Maximum number of search results to return (default: 10).")
    generate_answer: Optional[bool] = Field(default=False, description="Whether to generate an AI-powered natural language answer based on search results (default: False).")
    expand_query: Optional[bool] = Field(default=False, description="Generate query variations to improve recall (default: False).")
    rerank: Optional[bool] = Field(default=True, description="Reorder top candidate results for improved relevance (default: True).")
    temporal_relevance: Optional[float] = Field(default=None, description="Weight recent content higher than older content; 0 = no recency effect, 1 = only recent items matter (default: None).")
...
class AirweaveSearchTool(BaseTool):
    ...
    client: Optional[AirweaveSDK] = None
    async_client: Optional[AsyncAirweaveSDK] = None
    ...
    def _format_results(self, response: Any, query: str) -> str:
        ...
        for result in results:
            if isinstance(result, dict):
                ...
                if 'content' in formatted_result and isinstance(formatted_result['content'], str):
                    if len(formatted_result['content']) > self.max_content_length_per_result:
                        formatted_result['content'] = (
                            formatted_result['content'][:self.max_content_length_per_result] + "..."
                        )
    ...
    def _run(
        ...
        try:
            ...
            # Execute search
            logger.info(f"Searching Airweave collection '{collection_id}' with query: {query}")
            ...
            error_msg = f"Error searching Airweave collection '{collection_id}': {str(e)}"
