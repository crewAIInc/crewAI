...
def _validate_range(value: int, *, minimum: int, maximum: int, name: str) -> int:
    """Return an integer after enforcing inclusive API bounds."""
...
def _normalize_path_identifier(value: str, *, name: str) -> str:
    """Normalize and percent-encode one required API path identifier."""
...
def _require_text(value: str, *, name: str) -> str:
    """Trim and return a required text value."""
    cleaned = value.strip()
    if not cleaned:
        raise ValueError(f"{name} is required")
    return cleaned
...
def _error_body(response: requests.Response) -> str:
    """Return a bounded response body for an API error."""
    try:
...
class XquikToolBase(BaseTool):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize authenticated Xquik request settings."""
    ...
    def headers(self) -> dict[str, str]:
        """Return headers for the current Xquik API contract."""
        return {
    ...
    def _get(self, path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        """Fetch and decode one validated Xquik API resource."""
        try:
            response = safe_get(
...
class XquikSearchTweetsTool(XquikToolBase):
    ...
    def _run(
        ...
        """Search public posts with optional pagination and time bounds."""
...
class XquikGetTweetTool(XquikToolBase):
    ...
    def _run(self, tweet_id: str) -> dict[str, Any]:
        """Fetch one post by its path-safe identifier."""
...
class XquikGetUserTool(XquikToolBase):
    ...
    def _run(self, user: str) -> dict[str, Any]:
        """Fetch one user by username or identifier."""
...
class XquikGetUserTweetsTool(XquikToolBase):
    ...
    def _run(
        ...
        """Fetch a page of posts for one user."""
        normalized_user = _normalize_path_identifier(user, name="user")
...
class XquikGetTrendsTool(XquikToolBase):
    ...
    def _run(self, woeid: int = 1, count: int = 30) -> dict[str, Any]:
        """Fetch bounded regional trend results."""
        if woeid < 1:
