...
class URLReadTool(BaseTool):
    ...
    Security:
        ...
        The fetch pins TCP to an IP that passed validation. The returned text
        is still untrusted remote content flowing into an agent's context -- a
        fetched page can attempt to instruct the agent. Network egress policy
        and prompt-level handling cover that.
