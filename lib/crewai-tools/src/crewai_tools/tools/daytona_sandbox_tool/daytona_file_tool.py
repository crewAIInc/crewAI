...
class DaytonaFileToolSchema(BaseModel):
    action: FileAction = Field(
        ...
        description=(
            ...
            "'write' (uploads content to path), 'list' (lists a directory), "
            "'delete' (removes a file/dir), 'mkdir' (creates a directory), "
...
class DaytonaFileTool(DaytonaBaseTool):
    ...
    description: str = (
        ...
        "write content to a path, list a directory, delete a path, make a "
        "directory, or fetch file metadata."
    )
