...
MCP is an optional dependency, loaded when a Baizhi tool is called. Importing
`crewai_tools` or constructing these tools does not require the MCP extra;
calling them without it raises an installation error with the command above.
...
`accept_language`. This integration is limited to research: `download` accepts
only `False` and both page tools always send `download=False`. Exporting files
is outside this integration's scope, even though the hosted service supports it.
...
Rendered input validation errors omit rejected input values, including credentials
embedded in URLs.
