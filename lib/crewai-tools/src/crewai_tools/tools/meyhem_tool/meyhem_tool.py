...
class MeyhemSearchSchema(BaseModel):
    ...
    freshness: str | None = Field(None, description="Cache freshness: realtime, hour, day, week")
...
class MeyhemDiscoverSchema(BaseModel):
    ...
    ecosystem: str | None = Field(None, description="Filter by ecosystem: mcp, openclaw, or None for all")
...
class MeyhemSearchTool(BaseTool):
    ...
    def _run(self, query: str, max_results: int = 10, include_content: bool = False, freshness: str | None = None) -> str:
        payload = dict(query=query, max_results=max_results, include_content=include_content, agent_id=self.agent_id)
...
class MeyhemDiscoverTool(BaseTool):
    ...
    def _run(self, task: str, max_results: int = 5, ecosystem: str | None = None) -> str:
        ...
        try:
            ...
            r.raise_for_status()
            return json.dumps(r.json()["results"], indent=2)
        except Exception as e: return f"Discovery failed: {e}"
