...
Entroly exposes local context-selection and recovery tools over stdio MCP. Install
[`uv`](https://docs.astral.sh/uv/getting-started/installation/), then configure the
adapter to launch the published package without a persistent global install:
...
Before using it in a workflow, verify the installed package and local recovery
...
Context reduction and downstream task quality depend on the workload and selected
budget. Measure representative tasks before routing production requests.
