...
hooks, wrap an existing `BaseTool` before adding it to an agent. The third-party
[`decionis-crewai`](https://pypi.org/project/decionis-crewai/) adapter evaluates
the tool payload before execution while preserving the original tool schema:
...
Refusal decisions stop the inner tool and return a reference to signed decision
evidence. Set `shadow_mode=True` on the wrapper to observe decisions without
