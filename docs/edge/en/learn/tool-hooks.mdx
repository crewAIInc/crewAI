...
Raising `HookAborted` from the hook prevents the side effect from running.
The hook is scoped to `charge_payment`, so unrelated tools continue through the
normal CrewAI hook chain. In production, resolve `tenant_id` and `session_id`
from your authenticated request or job context, not from model-supplied tool
arguments.

Install Pramagent before running this example:
...
def resolve_policy_context(ctx: ToolCallHookContext) -> PolicyContext:
    """Replace this with your authenticated request or job-context lookup."""
    return PolicyContext(tenant_id="finance_team", session_id="crew-run-demo")


@on(InterceptionPoint.PRE_TOOL_CALL, tools=["charge_payment"])
def enforce_pramagent_policy(ctx: ToolCallHookContext) -> None:
    policy_context = resolve_policy_context(ctx)
    ...
    if decision.verdict == Verdict.ESCALATE:
        raise HookAborted(
            reason=f"Approval required before running {ctx.tool_name}: {decision.reason}",
            source="pramagent",
        )

    raise HookAborted(reason=decision.reason, source="pramagent")
```

The example treats `Verdict.ESCALATE` as a stop signal so the side effect does
not run automatically. For approval workflows, route that verdict to your human
approval path and re-run the tool only after approval is recorded. The hook
remains the pre-execution boundary.
