...
<Note>
    When no model is explicitly configured, CrewAI uses the first non-empty environment variable in this order: `MODEL`, `MODEL_NAME`, then `OPENAI_MODEL_NAME`. If none is set, it falls back to `DEFAULT_LLM_MODEL`, currently `gpt-5.6-luna`.
