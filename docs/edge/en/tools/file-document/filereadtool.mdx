...
For large files, use `start_line` and `line_count` to read only a range of lines.
...
# No default file — pass file_path when you call the tool
tool = FileReadTool()
content = tool.run(file_path='path/to/your/file.txt')

# OR set a default file at construction
tool = FileReadTool(file_path='path/to/your/file.txt')
content = tool.run()  # reads the default file

# Read only lines 100-149 of the default file
partial_content = tool.run(start_line=100, line_count=50)

# Read another file inside the sandbox
other = tool.run(file_path='path/to/other.txt')
```
