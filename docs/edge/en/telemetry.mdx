...
| Yes       | Trace Sharing Signal | Includes: that a batch of traces was successfully shared with CrewAI AMP, and whether it was shared anonymously (before you have an account) or linked to your account. Only the fact that a batch arrived is recorded - never the trace contents, crew or flow names, inputs, or outputs. No personal data. |
