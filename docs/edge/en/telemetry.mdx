...
| Yes       | Project Creation using crewAI CLI | Includes: that a new project was scaffolded by `crewai create`, which kind it was (`crew`, `json_crew` or `flow`), and the `project_id` minted for that new project and written into its own `pyproject.toml`. That is the new project's own ID, not the ID of the directory the command was run from -- the two can differ. No project name, no file contents, no code. No personal data. |
