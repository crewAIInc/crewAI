...
| `LLM(model=…)` | resolves |
|---|---|
| `gpt-4o-mini`, `my-gateway-model` | always — a bare name reaches the native provider whatever it is called |
| `openai/gpt-4o-mini`, `openai/o3-mini` | always — a native provider claims it |
| `openai/my-gateway-model` | **only where LiteLLM is installed** |

A prefixed name no native provider claims falls through to LiteLLM, and 1.x does not install it by
default (`crewai[litellm]` adds it). So the same line works on one machine and raises
`ImportError: … and the LiteLLM fallback package is not installed` on another. Using the bare form
for a gateway's own model names sidesteps the question entirely.
