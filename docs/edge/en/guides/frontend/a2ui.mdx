...
<Tip>
Choose [OpenUI](/edge/en/guides/frontend/openui) when you want the model to author the complete chat response as streaming OpenUI Lang and use OpenUI's `AgentInterface` as the frontend. Both approaches constrain the model to components you ship; A2UI integrates a catalog through the CopilotKit runtime, while OpenUI generates, parses, and renders OpenUI Lang over the CrewAI AG-UI stream.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="OpenUI" icon="window-maximize" href="/edge/en/guides/frontend/openui">
    Let the model compose a complete interactive response (open-ended).
  </Card>
</CardGroup>
