...
Because a Flow's state is streamed to the frontend automatically over AG-UI, you never wire up the transport yourself. You subclass the state to add your own fields, update them in the Flow, and read them in React.
...
Everything on `AgentState` is streamed to the frontend as it changes. That is the whole contract: write to `self.state`, and the UI sees it.
...
Wrapping the LLM call in `copilotkit_stream` streams tokens and state deltas to the frontend as they are produced, so the UI updates mid-run rather than only at the end.
