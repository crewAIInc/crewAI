...
On the frontend, call `useFrontendTool` with the same `name` the backend declared. Provide a `render` function and no handler: this tool is pure display, so there is nothing to execute in the browser.
...
- **`render` receives `{ args }`**, which fills in progressively as the model streams the call. Early on, `args` may be an empty object or hold only a partial field.
