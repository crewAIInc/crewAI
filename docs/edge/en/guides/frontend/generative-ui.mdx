...
CrewAI frontends render generative UI along a **spectrum**, from fully author-controlled (you decide every pixel) to agent-invented (the agent assembles the surface):
...
| **[Open-ended](#open-ended)** | The model authors the response as a UI program, or an external tool supplies it | [OpenUI](/edge/en/guides/frontend/openui), MCP tools |
...
At the far end, the model authors the response as a UI program instead of selecting one component or filling a fixed schema. With [OpenUI](/edge/en/guides/frontend/openui), a CrewAI Flow gives the model the exact component-library prompt and streams OpenUI Lang to `AgentInterface`. OpenUI parses and renders the response as charts, forms, follow-up choices, and other interactive components while it is still arriving. User actions return through the same Flow for another LLM turn.

The component library still bounds what the model can render, but the model controls how those components compose into the response. **MCP** offers another open-ended path: tools served by an MCP server render as tool calls in the chat, the same way backend tools do.
...
<CardGroup cols={2}>
  ...
  <Card title="OpenUI" icon="window-maximize" href="/edge/en/guides/frontend/openui">
    Stream model-authored charts, forms, and follow-ups (open-ended).
  </Card>
