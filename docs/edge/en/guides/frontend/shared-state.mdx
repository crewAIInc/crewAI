...
Two things make this shared rather than one-way: dumping `self.state` into the prompt means the agent always works from the latest recipe (including edits the user made in the UI), and assigning `self.state.recipe` streams the new value out to every connected client.
...
function RecipeEditor() {
  ...
  const setState = (s: any) => agent.setState(s); // write back to shared state
  ...
  return (
    <div>
      <input
        ...
        disabled={isLoading}
        onChange={(e) =>
          setState({ recipe: { ...recipe, title: e.target.value } })
        }
      />
...
- **Agent edits, UI updates.** The Flow assigns `self.state.recipe`, the new value streams to the client, and `OnStateChanged` re-renders your inputs.
