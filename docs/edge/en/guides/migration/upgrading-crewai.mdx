...
<Note>
  CrewAI requires `Python >=3.10, <3.15`. If `uv` was installed against an older interpreter, recreate the project venv with a supported Python before running `crewai install`.
</Note>
