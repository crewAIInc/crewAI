...
| Task `human_input` | Pauses for console input on the Task path. |
...
Require a human or an external policy check before irreversible, expensive, or public actions.
...
Enforce approval in code. Do not rely on the prompt alone.
