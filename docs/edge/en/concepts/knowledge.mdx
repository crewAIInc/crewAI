...
CrewAI exposes a provider-neutral RAG client abstraction for vector stores. The default provider is ChromaDB, and Qdrant and Milvus are supported as well. You can switch providers using configuration utilities.
...
<Note>
Install the optional provider extra before importing its configuration: `uv add "crewai[qdrant]"` for Qdrant or `uv add "crewai[milvus]"` for Milvus.
The default Milvus embedding function uses OpenAI embeddings, so set `OPENAI_API_KEY` before calling `add_documents`. Alternatively, pass a custom `embedding_function` to `MilvusConfig`.
</Note>
