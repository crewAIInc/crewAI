...
<AccordionGroup>
  ...
  <Accordion title="Pinstripes">
    [Pinstripes](https://pinstripes.io) is an OpenAI-compatible inference API. Because CrewAI routes LLM calls through LiteLLM, you can use Pinstripes as a drop-in provider — a single API key gives your agents access to DeepSeek, GLM, Qwen, and MiniMax models.

    Set the following environment variable in your `.env` file:
    ```toml Code
    PINSTRIPES_API_KEY=<your-api-key>
    ```
    ...
    llm = LLM(
        ...
        api_key="<your-pinstripes-api-key>",  # Or set PINSTRIPES_API_KEY
        temperature=0.7
    )
    ```

    Pinstripes uses the `openai/` LiteLLM prefix with a custom `base_url`, so the standard OpenAI-compatible transport is used.
    ...
    **Available models:**
    ...
    Use each model with the `openai/` prefix (e.g. `openai/ps/qwen3-35b`).
