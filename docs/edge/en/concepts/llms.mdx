...
<AccordionGroup>
  ...
  <Accordion title="Nebius Serverless Endpoint (vLLM)">
    ...
    <Warning>
      Tool use requires a model with tool-calling support and a vLLM deployment configured with
      the matching tool-call parser and automatic tool choice. Verify those options for your model
      before using CrewAI tools.
    </Warning>
