...
<AccordionGroup>
  ...
  <Accordion title="Nebius Serverless Endpoint (vLLM)">
    A Nebius Serverless Endpoint running vLLM exposes an OpenAI-compatible API. It uses the
    endpoint's managed HTTPS URL and bearer token, not `NEBIUS_API_KEY` from Nebius AI Studio.

    Set these values after creating the endpoint:
    ...
    `NEBIUS_MODEL_ID` must exactly match an ID returned by the endpoint's `/v1/models` route.
    Do not add a provider prefix such as `nebius/`.

    Before starting a Crew, wait for both the endpoint and the model to be ready:
    ...
    <Warning>
      Before running the tool-enabled example below, configure the Endpoint's vLLM arguments
      with automatic tool choice and the parser documented for your model:
      ...
      See the [vLLM tool-calling parser list](https://docs.vllm.ai/en/latest/features/tool_calling/#automatic-function-calling).
      These arguments are not required when your Crew does not use tools.
    </Warning>

    Then connect through CrewAI's native OpenAI-compatible client:
    ...
    If requests fail, check the following:

    - HTTP `401` or `403`: use the Endpoint bearer token, not a Nebius control-plane or AI Studio key.
    - HTTP `404` while starting: wait until the Endpoint is `RUNNING` and `/health` succeeds.
    - Model-not-found errors: copy the exact model ID from `/v1/models`.
    - Tool-call errors: confirm the model and vLLM tool parser support function calling.
    - Streaming errors: first retry with `stream=False` to separate endpoint streaming from model issues.

    See the [vLLM deployment guide for Nebius Serverless](https://docs.vllm.ai/en/latest/deployment/frameworks/nebius/)
    for prerequisites, endpoint creation, logs, and readiness checks. Stop or delete the Endpoint
    when it is no longer needed; closing the client does not stop the deployment or its billing.
  </Accordion>
</AccordionGroup>
