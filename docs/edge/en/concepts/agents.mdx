...
| **LLM** _(optional)_                    | `llm`                    | `Union[str, LLM, Any]`                | Language model that powers the agent. Uses the first non-empty environment variable: `MODEL`, `MODEL_NAME`, then `OPENAI_MODEL_NAME`; otherwise defaults to `gpt-5.6-luna`. |
...
- `llm`: Language model that powers the agent. Uses the first non-empty environment variable: `MODEL`, `MODEL_NAME`, then `OPENAI_MODEL_NAME`; otherwise defaults to `gpt-5.6-luna`.
