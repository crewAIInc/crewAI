...
SQLite is the default backend. Use `@persist` without an argument when local, file-based persistence is sufficient.
...
To choose SQLite explicitly, for example to control the database file location, pass a `SQLiteFlowPersistence` instance to `@persist`:
...
To use MongoDB, install the optional dependency, configure a connection string, and pass `MongoDbFlowPersistence` explicitly:
...
`MongoDbFlowPersistence` uses the `flow_persistence` database by default. Set `MONGODB_DATABASE` or pass `database_name` to choose another database. MongoDB persistence uses transactions, so the deployment must be MongoDB Atlas, a replica set, or a sharded cluster; standalone MongoDB servers are not supported.
