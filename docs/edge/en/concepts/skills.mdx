...
```shell Terminal
cd skills/my-skill
crewai skill publish            # private to your organization (default)
crewai skill publish --public   # anyone can install it
```

Publishing reads `name`, `description`, and `metadata.version` from the `SKILL.md` frontmatter and pushes the skill to the CrewAI registry under your organization. Useful flags:

| Flag | Effect |
| :--- | :--- |
| `--public` / `--private` | Visibility on the registry (private is the default). |
