...
CrewAI supports three types of guardrails:
...
2. **Deterministic arbitration guardrails**: Pydantic contracts (plus optional CEL rules and deadlines) that approve or dispute deliverables without an LLM judge — useful for agent-to-agent trust and escrow-style release gates.

3. **LLM-based guardrails**: String descriptions that use the agent's LLM to validate outputs based on natural language criteria. These are ideal for complex or subjective validation requirements.
...
### Deterministic Arbitration Guardrails
...
from crewai import Task, ArbitrationGuardrail

class HotelBooking(BaseModel):
    price_per_night: float = Field(ge=0, le=180)
    room_type: str
...
escrow_guardrail = ArbitrationGuardrail(
    HotelBooking,
    rules=[
        "output.price_per_night >= 0.0 && output.price_per_night <= 180.0",
        "output.non_smoking == true",
    ],
    deadline=datetime.now(timezone.utc) + timedelta(hours=1),
)

booking_task = Task(
    description="Book a non-smoking hotel room at most $180/night",
    expected_output="JSON booking confirmation matching HotelBooking",
    agent=booking_agent,
    guardrail=escrow_guardrail,
)
```

Outcomes are deterministic: `APPROVED` when every hard constraint passes, or
`DISPUTED` with structured violations (invalid JSON, missing fields, failed
rules, missed deadline). Use this as the release gate before marking a
delegated task resolved or releasing funds on an external payment rail.
