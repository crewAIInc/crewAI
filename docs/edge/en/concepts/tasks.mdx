...
class HotelBooking(BaseModel):
    price_per_night: float = Field(le=180)
    room_type: str
...
booking_task = Task(
    description="Book a non-smoking hotel room under $180/night",
...
`DISPUTED` with field-level violations (invalid JSON, missing fields, failed
