...
Any OpenAI-compatible gateway or proxy can sit in front of memory LLM calls by changing the model's `base_url`. Use this when you want a gateway to observe or route the same LLM traffic CrewAI already sends while keeping CrewAI's native `Memory` API, storage, scopes, and retrieval behavior unchanged.
...
You can also configure the same gateway globally for OpenAI-compatible calls, then keep passing `default_headers` on the `LLM` if the gateway needs a user or session identifier. Replace the example URL with your gateway endpoint:

```bash
export OPENAI_BASE_URL="https://your-gateway.example.com/v1"
```

CrewAI does not need a gateway-specific SDK or runtime dependency for this path. The integration boundary is the OpenAI-compatible LLM endpoint: CrewAI continues to manage local memory persistence, `root_scope` isolation, explicit `scope` values, forgetting, and recall scoring; the gateway receives the proxied LLM calls and can use headers such as `x-session-id` to link interactions to a specific end user.
