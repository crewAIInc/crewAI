...
- **Dakera (Self-Hosted Persistent Memory)**: Network-based persistent memory server supporting shared cross-crew memory, multi-agent context isolation, and decay-weighted recall. Install `crewai-dakera` and pass `DakeraStorageBackend` to `Memory(storage=...)`.
...
**Why self-hosted storage**: Unlike local-only file storage, network services like Dakera allow multiple crews, agents, or distributed worker nodes to share a central persistent vector memory store.
