...
- **Community backends**: [`crewai-memory`](https://github.com/skamalj/crewai-memory) provides `StorageBackend` implementations for Amazon DynamoDB ([`crewai-memory-dynamodb`](https://pypi.org/project/crewai-memory-dynamodb/)), Azure Cosmos DB ([`crewai-memory-cosmosdb`](https://pypi.org/project/crewai-memory-cosmosdb/)), Google Firestore ([`crewai-memory-firestore`](https://pypi.org/project/crewai-memory-firestore/)) and PostgreSQL with pgvector ([`crewai-memory-postgres`](https://pypi.org/project/crewai-memory-postgres/)), each using the database's own vector search. They are maintained outside this repository.
