...
- **Dakera Persistent Memory**: Network-based persistent memory server powered by [`crewai-dakera`](https://pypi.org/project/crewai-dakera/) ([Source Repository](https://github.com/Dakera-AI/dakera-crewai)). Handles both content and query embeddings server-side with hybrid vector/BM25 retrieval, eliminating the need for a local embedding model.
