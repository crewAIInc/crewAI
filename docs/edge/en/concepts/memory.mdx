...
### Mimir — Persistent Cross-Session Memory (Community)

[Mimir](https://github.com/Perseus-Computing-LLC/mimir) is an **MCP-native, local-first, zero-dependency persistent memory engine** that gives CrewAI agents durable memory across sessions. It runs as a single static Rust binary (~8 MB) with embedded SQLite — no Docker, no Postgres, no cloud dependencies.

**Key capabilities for CrewAI:**

- **36 MCP tools** — agents access memory directly over MCP stdio/HTTP using `mimir_remember`, `mimir_recall`, `mimir_search_memories`, and more.
- **Hybrid search** — BM25 (FTS5) + dense embeddings with Reciprocal Rank Fusion (RRF) for precise recall.
- **Confidence decay** — memories naturally fade unless reinforced, preventing stale context from crowding out useful information.
- **AES-256-GCM encryption** at rest for production deployments.
- **Entity graph** — link and traverse related memories.
- **Immutable journal audit trail** — every operation is recorded.

**Getting started:**

Install Mimir (one command, zero dependencies):

```bash
curl -sSf https://raw.githubusercontent.com/Perseus-Computing-LLC/mimir/main/scripts/install.sh | sh
```

**Option 1 – Agent-managed (recommended):** CrewAI launches Mimir for you via MCP stdio. No manual server startup needed.
...
**Option 2 – Standalone server:** Start Mimir manually, then connect from CrewAI (useful for shared memory across multiple agent processes, or direct `mimir` CLI inspection).

```bash
# Start Mimir (in a separate terminal or as a daemon)
mimir serve --db ./crew_memory.db --port 9100 --transport http
```
...
Choose one approach — Option 1 for simplicity, Option 2 for shared or multi-process memory.

For programmatic access via the `MimirMemory` adapter (implements CrewAI's `StorageBackend` protocol), see the [CrewAI integration example](https://github.com/Perseus-Computing-LLC/mimir/blob/main/examples/crewai_integration.py) in Mimir's repository.
