...
<Note>
    عندما لا يتم تحديد نموذج صراحةً، يستخدم CrewAI أول متغير بيئة غير فارغ بالترتيب التالي: `MODEL`، ثم `MODEL_NAME`، ثم `OPENAI_MODEL_NAME`. إذا لم يتم تعيين أي منها، يستخدم القيمة الاحتياطية `DEFAULT_LLM_MODEL`، وهي حاليًا `gpt-5.6-luna`.
