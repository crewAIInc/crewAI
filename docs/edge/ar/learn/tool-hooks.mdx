...
### 9. بوابة سياسة خارجية قبل التنفيذ

عندما تحتاج إلى سياسة مركزية قابلة للتدقيق بدلاً من قواعد محلية داخل الخطافات، لف أداة `BaseTool` موجودة قبل إضافتها إلى الوكيل. يقيّم محول [`decionis-crewai`](https://pypi.org/project/decionis-crewai/0.1.0/) الخارجي المنشور حمولة الأداة قبل التنفيذ مع الحفاظ على مخطط الأداة الأصلي. ثبّت الإصدار الموثق أولاً:
...
ترفع قرارات `BLOCK` و`REVIEW_REQUIRED` و`ESCALATE` وأخطاء السياسة `ERROR` الاستثناء المُصدّر `DecionisGuardRefusal` قبل تشغيل الأداة الداخلية. يتضمن الاستثناء مرجعاً إلى دليل قرار موقّع. استخدم `shadow_mode=True` في الغلاف لمراقبة القرارات من دون تغيير سلوك التنفيذ.
