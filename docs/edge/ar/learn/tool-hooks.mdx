...
عندما تحتاج إلى سياسة مركزية قابلة للتدقيق بدلاً من قواعد محلية داخل الخطافات، لف أداة `BaseTool` موجودة قبل إضافتها إلى الوكيل. يقيّم محول [`decionis-crewai`](https://pypi.org/project/decionis-crewai/) الخارجي حمولة الأداة قبل التنفيذ مع الحفاظ على مخطط الأداة الأصلي:
...
تمنع قرارات الرفض تنفيذ الأداة الداخلية وتعيد مرجعاً إلى دليل قرار موقّع. استخدم `shadow_mode=True` في الغلاف لمراقبة القرارات من دون تغيير سلوك التنفيذ.
