---
title: "تكامل Datadog"
description: "راقب عمليات نشر CrewAI AMP المُستضافة ذاتيًا في Datadog — اختر مسار وكيل Datadog لشحن السجلات بتكلفة فعّالة أو استيعاب OTLP من Datadog للإعداد بدون وكيل، ثم استورد لوحة معلومات العمليات الجاهزة."
icon: "dog"
mode: "wide"
---
...
<Note>
For vendor-neutral observability via any OTLP backend (Grafana, Honeycomb, your own collector), see [OpenTelemetry Export](/ar/enterprise/guides/capture_telemetry_logs).
</Note>
...
<Info>
This schema applies to the **Datadog Agent path** — stdout JSON logs produced when `CREWAI_LOG_FORMAT=json` is set. Logs delivered via the **Datadog OTLP intake** use OpenTelemetry attribute names and may differ; see [OpenTelemetry Export](/ar/enterprise/guides/capture_telemetry_logs).
</Info>
