...
### اختيار واجهة الاستمرارية

SQLite هي واجهة التخزين الافتراضية. استخدم `@persist` بدون وسيطة عندما تكون الاستمرارية المحلية المعتمدة على ملف كافية.
...
لاختيار SQLite صراحةً، مثل التحكم في موقع ملف قاعدة البيانات، مرّر نسخة من `SQLiteFlowPersistence` إلى `@persist`:
...
لاستخدام MongoDB، ثبّت التبعية الاختيارية، واضبط سلسلة الاتصال، ثم مرّر `MongoDbFlowPersistence` صراحةً:
...
يستخدم `MongoDbFlowPersistence` قاعدة بيانات `flow_persistence` افتراضيًا. اضبط `MONGODB_DATABASE` أو مرّر `database_name` لاختيار قاعدة بيانات أخرى. تستخدم استمرارية MongoDB المعاملات، لذا يجب أن يكون النشر MongoDB Atlas أو مجموعة متماثلة أو عنقودًا مجزأً؛ لا تدعم خوادم MongoDB المستقلة.
