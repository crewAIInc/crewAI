...
<AccordionGroup>
  ...
  <Accordion title="Nebius Serverless Endpoint (vLLM)">
    يوفّر Nebius Serverless Endpoint الذي يشغّل vLLM واجهة API متوافقة مع OpenAI. ويستخدم عنوان HTTPS المُدار للـ Endpoint ورمز bearer الخاص به، وليس `NEBIUS_API_KEY` الخاص بـ Nebius AI Studio.

    عيّن القيم التالية بعد إنشاء Endpoint:
    ...
    يجب أن يطابق `NEBIUS_MODEL_ID` تمامًا أحد المعرّفات التي يعيدها مسار `/v1/models` في Endpoint. لا تضف بادئة مزوّد مثل `nebius/`.

    قبل تشغيل Crew، انتظر حتى يصبح كل من Endpoint والنموذج جاهزين:
    ...
    <Warning>
      قبل تشغيل المثال الذي يستخدم الأدوات أدناه، اضبط معاملات vLLM للـ Endpoint لتفعيل الاختيار التلقائي للأدوات واستخدام المحلل الموثّق لنموذجك:
      ...
      راجع [قائمة محللات tool calling في vLLM](https://docs.vllm.ai/en/latest/features/tool_calling/#automatic-function-calling).
      لا تكون هذه المعاملات مطلوبة عندما لا تستخدم Crew أدوات.
    </Warning>

    ثم اتصل عبر عميل CrewAI الأصلي المتوافق مع OpenAI:
    ...
    إذا فشلت الطلبات، فتحقّق مما يلي:

    - HTTP `401` أو `403`: استخدم رمز bearer الخاص بالـ Endpoint، وليس رمز مستوى التحكم في Nebius أو مفتاح AI Studio.
    - HTTP `404` أثناء بدء التشغيل: انتظر حتى يصبح Endpoint في حالة `RUNNING` وينجح `/health`.
    - أخطاء عدم العثور على النموذج: انسخ معرّف النموذج بدقة من `/v1/models`.
    - أخطاء tool-call: تأكد من أن النموذج ومحلل أدوات vLLM يدعمان استدعاء الدوال.
    - أخطاء البث: أعد المحاولة أولًا باستخدام `stream=False` للفصل بين مشاكل بث Endpoint ومشاكل النموذج.

    راجع [دليل نشر vLLM على Nebius Serverless](https://docs.vllm.ai/en/latest/deployment/frameworks/nebius/) لمعرفة المتطلبات وإنشاء Endpoint والسجلات وفحوص الجاهزية. أوقف Endpoint أو احذفه عند عدم الحاجة إليه؛ إغلاق العميل لا يوقف النشر أو الفوترة.
  </Accordion>
</AccordionGroup>
