...
<Note>
    모델을 명시적으로 설정하지 않으면 CrewAI는 `MODEL`, `MODEL_NAME`, `OPENAI_MODEL_NAME` 순서로 환경 변수를 확인하여 비어 있지 않은 첫 번째 값을 사용합니다. 모두 설정되지 않은 경우 `DEFAULT_LLM_MODEL` 값으로 폴백하며, 현재 값은 `gpt-5.6-luna`입니다.
