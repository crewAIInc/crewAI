...
Entroly는 stdio MCP를 통해 로컬 컨텍스트 선택 및 복구 도구를 제공합니다.
[`uv`](https://docs.astral.sh/uv/getting-started/installation/)를 설치한 다음,
영구 전역 설치 없이 검증된 Entroly 버전을 실행하도록 어댑터를 구성합니다.
...
워크플로에서 사용하기 전에 API 키 없이 설치된 패키지와 로컬 복구 경로를
검증합니다.
...
컨텍스트 감소율과 후속 작업 품질은 워크로드와 선택한 예산에 따라 달라집니다.
프로덕션 요청을 라우팅하기 전에 대표 작업을 측정하세요.
