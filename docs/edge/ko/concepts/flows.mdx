...
SQLite가 기본 백엔드입니다. 로컬 파일 기반 지속성으로 충분한 경우 인자 없이 `@persist`를 사용하세요.
...
데이터베이스 파일 위치를 제어하는 것처럼 SQLite를 명시적으로 선택하려면 `SQLiteFlowPersistence` 인스턴스를 `@persist`에 전달하세요.
...
MongoDB를 사용하려면 선택 의존성을 설치하고 연결 문자열을 설정한 뒤 `MongoDbFlowPersistence`를 명시적으로 전달하세요.
...
`MongoDbFlowPersistence`는 기본적으로 `flow_persistence` 데이터베이스를 사용합니다. 다른 데이터베이스를 선택하려면 `MONGODB_DATABASE`를 설정하거나 `database_name`을 전달하세요. MongoDB 지속성은 트랜잭션을 사용하므로 MongoDB Atlas, 레플리카 세트 또는 샤딩 클러스터가 필요합니다. 독립 실행형 MongoDB 서버는 지원되지 않습니다.
