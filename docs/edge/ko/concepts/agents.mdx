...
| **LLM** _(옵션)_                        | `llm`                    | `Union[str, LLM, Any]`        | 에이전트를 구동하는 언어 모델입니다. `MODEL`, `MODEL_NAME`, `OPENAI_MODEL_NAME` 순서로 비어 있지 않은 첫 환경 변수를 사용하며, 없으면 `gpt-5.6-luna`를 사용합니다. |
...
- `llm`: 에이전트를 구동하는 언어 모델입니다. `MODEL`, `MODEL_NAME`, `OPENAI_MODEL_NAME` 순서로 비어 있지 않은 첫 환경 변수를 사용하며, 없으면 `gpt-5.6-luna`를 사용합니다.
