...
- `timeout`은 클라이언트의 대기 시간만 제한하며 원격 프로세스 종료를 **보장하지 않습니다**. 비동기 취소도 종료를 보장하지 않습니다. 시간 초과나 연결 실패 후에는 부수 효과가 있는 명령을 다시 실행하기 전에 Sprite를 확인하세요.
- 출력은 SDK가 캡처한 **후에** 잘립니다. 이는 에이전트 컨텍스트를 제한하지만 메모리 사용량은 제한하지 않습니다. 많은 출력은 Sprite 내부 파일로 리디렉션하세요.
