...
<Note>
    Quando nenhum modelo é configurado explicitamente, o CrewAI usa a primeira variável de ambiente não vazia nesta ordem: `MODEL`, `MODEL_NAME` e `OPENAI_MODEL_NAME`. Se nenhuma estiver definida, usa o valor de `DEFAULT_LLM_MODEL`, atualmente `gpt-5.6-luna`.
