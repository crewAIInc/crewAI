...
### 8. Gate de Política Externo Antes da Execução

Quando você precisa de uma política central e auditável em vez de regras locais nos hooks, envolva uma `BaseTool` existente antes de adicioná-la ao agente. O adaptador de terceiros publicado [`decionis-crewai`](https://pypi.org/project/decionis-crewai/0.1.0/) avalia o payload da ferramenta antes da execução e preserva o schema original. Primeiro, instale a versão documentada:

```bash
pip install "decionis-crewai==0.1.0"
```
...
guarded_tool = DecionisGuardedTool.wrap(
    inner_tool=existing_tool,
    client=DecionisClient(api_key="..."),
    tenant_id="your-org-uuid",
    workflow_key="tool_execution",
)
agent = Agent(..., tools=[guarded_tool])
```

Os resultados `BLOCK`, `REVIEW_REQUIRED`, `ESCALATE` e o erro de política `ERROR` lançam a exceção exportada `DecionisGuardRefusal` antes da execução da ferramenta interna. A exceção inclui uma referência para a evidência de decisão assinada. Use `shadow_mode=True` no wrapper para observar decisões sem alterar o comportamento da execução.
