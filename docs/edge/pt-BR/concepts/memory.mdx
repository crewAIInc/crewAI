...
O CrewAI suporta backends de armazenamento plugáveis para persistir e consultar a memória unificada.
...
- **Memória Persistente Dakera**: Servidor de memória persistente em rede alimentado pelo pacote [`crewai-dakera`](https://pypi.org/project/crewai-dakera/) ([Repositório Fonte](https://github.com/Dakera-AI/dakera-crewai)). Gerencia o embedding de conteúdo e consultas no lado do servidor com busca híbrida vetor/BM25 e recall ponderado por decaimento, eliminando a necessidade de um modelo de embedding local.
...
### Exemplo de Integração com o Dakera
...
storage = DakeraStorage(
    ...
    agent_id="my-agent-id",  # O mesmo agent_id permite compartilhamento de memória; IDs distintos isolam os namespaces de contexto
)

# Salve e busque memórias persistentes diretamente no servidor Dakera
storage.save(value="Agent learned API rate limit is 60 req/min")
results = storage.search(query="API rate limit", limit=5)
```

**Por que usar armazenamento auto-hospedado**: Serviços de rede como o Dakera permitem que múltiplas crews, agentes ou nós de trabalho distribuídos compartilhem um repositório central de memória vetorial e BM25 persistente sem depender apenas do armazenamento de arquivos local. Valores de `agent_id` correspondentes compartilham memórias entre sessões, enquanto valores de `agent_id` diferentes isolam namespaces de memória no servidor.
