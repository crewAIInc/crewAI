...
- **Dakera (Memória Persistente Auto-Hospedada)**: Servidor de memória persistente em rede que suporta memória compartilhada entre crews, isolamento de contexto multi-agente e recall ponderado por decaimento. Instale `crewai-dakera` e passe `DakeraStorageBackend` para `Memory(storage=...)`.
...
**Por que armazenamento auto-hospedado**: Ao contrário do armazenamento de arquivos apenas local, serviços de rede como o Dakera permitem que múltiplas crews, agentes ou nós de trabalho distribuídos compartilhem um repositório central de memória vetorial persistente.
