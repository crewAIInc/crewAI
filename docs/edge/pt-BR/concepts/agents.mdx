...
| **LLM** _(opcional)_                    | `llm`                    | `Union[str, LLM, Any]`                | Modelo de linguagem que alimenta o agente. Usa a primeira variável de ambiente não vazia: `MODEL`, `MODEL_NAME` e `OPENAI_MODEL_NAME`; caso contrário, usa `gpt-5.6-luna`. |
...
- `llm`: Modelo de linguagem que alimenta o agente. Usa a primeira variável de ambiente não vazia: `MODEL`, `MODEL_NAME` e `OPENAI_MODEL_NAME`; caso contrário, usa `gpt-5.6-luna`.
