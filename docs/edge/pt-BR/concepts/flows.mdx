...
### Escolhendo um Backend de Persistência

SQLite é o backend padrão. Use `@persist` sem argumento quando a persistência local baseada em arquivo for suficiente.
...
Para escolher SQLite explicitamente, por exemplo para controlar o local do arquivo de banco de dados, passe uma instância de `SQLiteFlowPersistence` para `@persist`:
...
Para usar MongoDB, instale a dependência opcional, configure uma string de conexão e passe `MongoDbFlowPersistence` explicitamente:

```bash
uv add 'crewai[mongodb]'
# Or: pip install 'crewai[mongodb]'
export MONGODB_CONNECTION_STRING='mongodb+srv://<user>:<password>@<cluster>/'
```

```python
from crewai.flow.persistence import MongoDbFlowPersistence, persist
...
`MongoDbFlowPersistence` usa o banco de dados `flow_persistence` por padrão. Defina `MONGODB_DATABASE` ou passe `database_name` para escolher outro banco de dados. A persistência MongoDB usa transações, portanto a implantação deve ser MongoDB Atlas, um conjunto de réplicas ou um cluster fragmentado; servidores MongoDB autônomos não são compatíveis.
