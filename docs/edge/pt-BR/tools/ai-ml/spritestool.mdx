...
- `timeout` limita o tempo de espera do cliente; **não** garante o encerramento do processo remoto. O cancelamento assíncrono também não garante o encerramento. Após um timeout ou falha de conexão, inspecione o Sprite antes de repetir um comando com efeitos colaterais.
- A saída é truncada **depois** de ser capturada pelo SDK. Isso limita o contexto do agente, não o consumo de memória. Redirecione saídas extensas para arquivos no Sprite.
