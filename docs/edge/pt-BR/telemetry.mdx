...
| Sim    | Criação de Projeto pelo CLI do crewAI | Inclui: o fato de um novo projeto ter sido criado por `crewai create`, de qual tipo ele é (`crew`, `json_crew` ou `flow`) e o `project_id` gerado para ele naquele momento. Sem nome de projeto, sem conteúdo de arquivos, sem código. Sem dados pessoais. |
