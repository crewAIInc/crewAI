...
| Sim    | Criação de Projeto pelo CLI do crewAI | Inclui: o fato de um novo projeto ter sido criado por `crewai create`, de qual tipo ele é (`crew`, `json_crew` ou `flow`) e o `project_id` gerado para esse novo projeto e gravado no `pyproject.toml` dele. É o ID do próprio projeto novo, não o ID do diretório de onde o comando foi executado -- os dois podem diferir. Sem nome de projeto, sem conteúdo de arquivos, sem código. Sem dados pessoais. |
