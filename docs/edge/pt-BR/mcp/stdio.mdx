...
O Entroly disponibiliza ferramentas locais de seleção e recuperação de contexto
por stdio MCP. Instale o
[`uv`](https://docs.astral.sh/uv/getting-started/installation/) e configure o
adaptador para executar a versão verificada do Entroly sem uma instalação global
persistente:
...
# Run this example from the repository Entroly should index, or replace this
# value with that repository's absolute path.
project_root = os.getcwd()

server_params = StdioServerParameters(
    command="uvx",
    args=["--from", "entroly==1.0.77", "entroly", "serve"],
    env={
        **os.environ,
        "ENTROLY_NO_DOCKER": "1",
        "ENTROLY_SOURCE": project_root,
    },
)

with MCPServerAdapter(server_params) as tools:
    # Pass the discovered MCP tools to an Agent through its `tools` argument.
    print([tool.name for tool in tools])
```

Antes de usá-lo em um fluxo de trabalho, verifique o pacote instalado e o
caminho de recuperação local sem uma chave de API:

```bash
uvx --from 'entroly==1.0.77' entroly verify-claims --max-files 30
```

A redução de contexto e a qualidade das tarefas posteriores dependem da carga de
trabalho e do orçamento selecionado. Meça tarefas representativas antes de
encaminhar solicitações de produção.
