...
| **PII detection applied** | `Yes` if a per-deployment PII config or a matching [PII policy](/en/enterprise/features/agent-control-plane/policies) is active. |
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Control Plane — Policies" icon="shield-check" href="/en/enterprise/features/agent-control-plane/policies">
