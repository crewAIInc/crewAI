...
LLM 기반 라우터와 라우트별 핸들러로 멀티턴 챗을 만들고 싶지만 라이프사이클을 직접 배선하고 싶지 않을 때 사용하세요. 완전한 제어가 필요하면 위의 `Flow[ChatState]`로 내려가세요. `crewai.experimental.ConversationalFlow`는 `Flow[ConversationState]` + `conversational = True`의 한 줄 별칭으로 계속 동작하며 — 레거시 임포트를 위해 유지되지만, 새 코드에서는 명시적 형태를 권장합니다.
