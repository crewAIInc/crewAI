...
# 일반 파일 (타입 자동 감지)
file = File(source="document.pdf")  # PDF로 감지됨
file = File(source="photo.jpg")     # 이미지로 감지됨
```
