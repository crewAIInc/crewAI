...
<AccordionGroup>
  ...
  <Accordion title="Tzafon">
    CrewAI는 OpenAI 호환 Tzafon API를 통해 [Tzafon](https://tzafon.ai)과 네이티브 통합을 제공합니다.
    ...
    **기본 사용법:**
    ...
    llm = LLM(
        ...
        api_key="sk_your_api_key",  # 또는 TZAFON_API_KEY 환경 변수 설정
        temperature=0.7
    )
    ```
    ...
    **고급 구성:**
    ...
    llm = LLM(
        ...
        base_url="https://api.tzafon.ai/v1",  # 선택적 커스텀 엔드포인트
        temperature=0.7,
        max_tokens=4000,
        stop=["END"],
    ...
    **지원되는 환경 변수:**
    - `TZAFON_API_KEY`: Tzafon API 키 (필수)
    - `TZAFON_BASE_URL`: 커스텀 기본 URL (선택 사항, 기본값: `https://api.tzafon.ai/v1`)
    ...
    **지원 모델:**
    ...
    | 모델                         | 최적 용도                                    | 가격 (입력 / 출력)              |
    |-----------------------------|----------------------------------------------|--------------------------------|
    | tzafon.sm-1                 | 범용 작업, 빠른 응답                          | $0.20 / $0.30 (100만 토큰당)   |
    | tzafon.northstar-cua-fast   | 브라우저 및 데스크톱 자동화 에이전트            | $0.30 / $1.00 (100만 토큰당)   |

    <Info>
      Tzafon의 API는 OpenAI SDK와 완전히 호환됩니다. `base_url`을 변경하여 OpenAI Python SDK로 직접 사용할 수도 있습니다:
