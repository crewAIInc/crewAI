...
<Note>
**Versions.** These guides use `ag-ui-crewai` (the AG-UI-native CrewAI package, imported as `ag_ui_crewai`), `@ag-ui/crewai`, and `@copilotkit/*` at **1.61.2+** (the `@copilotkit/react-core/v2` API). CopilotKit's own CrewAI docs may show the older `copilotkit` / `copilotkit.crewai` Python package and the pre-v2 hooks; the two are different generations, so pin the versions above when following this section.
</Note>
