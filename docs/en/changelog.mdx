...
<Update label="Feb 26, 2026">
  ...
  - Introduce New Unified Memory System
  - Add yanked detection for version notes
  - Add version check & integrate update notices
  ...
  - Fix bedrock region to respect the environment variable
  - Ensure callbacks are ran/awaited if promise
  - Capture method name in exception context
  - Preserve enum type in router result and improve types
  - Fix cyclic flows breaking when persistence ID is passed in inputs
  ...
  - Fix tool error causing double event scope pop
  - Resolve race condition in guardrail event emission test
  - Relax openai and litellm dependency constraints
  ...
  ### Refactoring
  - Refactor crew to provider pattern
