...
# Advanced VoyageAI configuration with all options
crew = Crew(
    agents=[agent],
    tasks=[...],
    knowledge_sources=[knowledge_source],
    embedder={
        ...
        "config": {
            "api_key": "your-voyage-api-key",
            "model": "voyage-4",
            "output_dimension": 1024,  # Optional: Custom embedding dimension (default: model-specific)
            "input_type": "document",  # Optional: "document" or "query" (default: None)
            "truncation": True,  # Optional: Truncate long texts (default: True)
            "max_retries": 3,  # Optional: Max API retry attempts (default: 0)
            "timeout": 30.0  # Optional: API timeout in seconds (default: None)
        }
    }
)

# Using contextual embeddings for better semantic understanding
crew = Crew(
    agents=[agent],
    tasks=[...],
    knowledge_sources=[knowledge_source],
    embedder={
        ...
        "config": {
            "api_key": "your-voyage-api-key",
            "model": "voyage-context-3",  # Contextual embedding model
...
<Tip>
**VoyageAI Models Overview**

VoyageAI offers several embedding models optimized for different use cases:

**Voyage-4 Series (Latest):**
- `voyage-4` (Recommended): Latest general-purpose model with high performance (320K batch tokens)
- `voyage-4-lite`: Best cost-performance ratio with highest throughput (1M batch tokens)
- `voyage-4-large`: Premium quality model for demanding retrieval tasks (120K batch tokens)

**Voyage-3.5 Series:**
- `voyage-3.5`: Previous generation general-purpose model
- `voyage-3.5-lite`: Lightweight version with 1M token context

**Voyage-3 Series:**
- `voyage-3`: General-purpose model
- `voyage-3-lite`: Lightweight version of voyage-3
- `voyage-3-large`: Larger model for best quality (120K tokens)

**Specialized Models:**
- `voyage-context-3`: Contextual embeddings for multi-document understanding (32K tokens)
...
- `voyage-multimodal-3.5`: Latest multimodal embeddings
...
**API Key:**
Get your API key from [VoyageAI Dashboard](https://dash.voyageai.com/)
...
knowledge = StringKnowledgeSource(
    content="CrewAI is a powerful framework for building multi-agent AI systems. "
            "It supports various embedding providers including VoyageAI, which offers "
            "state-of-the-art embedding models optimized for retrieval tasks."
)

# Create agent (works great with Claude)
agent = Agent(
    role="AI Research Assistant",
    goal="Provide accurate information about AI frameworks",
    backstory="Expert in AI and machine learning frameworks",
    llm=LLM(provider="anthropic", model="claude-3-sonnet-20240229")
)

# Create task
task = Task(
    description="Explain what CrewAI is and its key features",
    agent=agent,
    expected_output="A concise explanation of CrewAI"
)

# Create crew with VoyageAI embeddings
crew = Crew(
    agents=[agent],
    tasks=[task],
