...
file = File(source="document.pdf")  # Detected as PDF
file = File(source="photo.jpg")     # Detected as Image
```
...
# Strict: Raise error if file exceeds limits
image = ImageFile(source="large.png", mode="strict")

# Auto: Automatically resize/chunk to fit limits (default)
image = ImageFile(source="large.png", mode="auto")

# Warn: Log warning but attempt to send anyway
image = ImageFile(source="large.png", mode="warn")

# Chunk: Split large files into multiple chunks
