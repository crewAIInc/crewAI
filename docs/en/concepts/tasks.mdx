...
| **Proof Links**   | `proof_links`   | `dict[str, str]`           | Optional receipt/proof links (for example proof, preview, and download URLs).                     |
...
### Optional Receipt/Proof Links

You can attach verifiable receipt links to each `TaskOutput` through a callback.
This is useful for wrappers such as the
[`claw2claw-receipt-wrapper`](https://github.com/jacobtop-tcg/claw2claw-receipt-wrapper),
where each completed task can publish auditable proof URLs.

```python Code
from crewai import Agent, Crew, Process, Task, TaskOutput

def attach_proof_links(output: TaskOutput) -> None:
    # Example integration point: call your wrapper here.
    # receipt = mint_receipt(output.raw)
    output.proof_links = {
...
researcher = Agent(
    role="Researcher",
    goal="Find high-signal updates",
    backstory="You summarize updates clearly.",
)

research_task = Task(
    description="Research the latest CrewAI updates.",
...
crew = Crew(
    ...
    tasks=[research_task],
    process=Process.sequential,
    task_callback=attach_proof_links,  # Runs after every task
)

result = crew.kickoff()
print(result.tasks_output[0].proof_links["proof"])
```
