...
| **Action Based** _(optional)_    | `action_based`    | `bool`                        | Whether the task is action-oriented (execute tools) or text-oriented. Defaults to True.                             |
| **Expected Output** _(optional)_ | `expected_output` | `Optional[str]`               | A detailed description of what the task's completion looks like. Only required for text-oriented tasks.             |
...
## Action-Oriented vs Text-Oriented Tasks

CrewAI supports two types of tasks based on their intended outcome:

### Action-Oriented Tasks (Default)

Action-oriented tasks are designed for agents that need to **execute actions** using tools rather than produce text output. This is the default behavior (`action_based=True`).

```python Code
from crewai import Task

# The agent will use tools to execute the task
# No expected_output needed - the action IS the result
buy_task = Task(
    description="Buy 100 BTC on Kraken at market price",
    agent=trader_agent
)
...
portfolio_task = Task(
    ...
    2. Get the current BTC/EUR price
    3. Place a limit buy order for 0.5 BTC at 5% below market price
    """,
    agent=trader_agent
)
```
...
Text-oriented tasks are for when you need the agent to **produce text output** like reports, summaries, or analyses. Set `action_based=False` and provide `expected_output`.

```python Code
from crewai import Task

# The agent will produce a text output
analysis_task = Task(
    description="Analyze the Bitcoin market trends for the past month",
    expected_output="A detailed analysis report with key insights and predictions",
    action_based=False,
    agent=analyst_agent
)
```
...
| Use Case | Task Type | `action_based` | `expected_output` |
| :------- | :-------- | :------------- | :---------------- |
| Execute trades | Action | `True` (default) | Not needed |
| Send emails | Action | `True` (default) | Not needed |
| Create files | Action | `True` (default) | Not needed |
| API operations | Action | `True` (default) | Not needed |
| Write reports | Text | `False` | Required |
| Generate summaries | Text | `False` | Required |
| Research & analysis | Text | `False` | Required |

<Note type="info" title="Tracking Executed Actions">
For action-oriented tasks, all tool executions are automatically tracked in `task.executed_actions` and `task.output.actions_executed`. This allows you to see exactly what actions the agent performed.
</Note>
...
| **Actions Executed**  | `actions_executed`  | `list[dict]`               | List of tool actions executed during the task. Each dict contains tool name, arguments, result, and success status. |
| **Execution Success** | `execution_success` | `bool`                     | Whether the task execution was successful. Defaults to True.                                       |
...
# For action-oriented tasks, check executed actions
if task_output.actions_executed:
    print(f"Actions Executed: {len(task_output.actions_executed)}")
    for action in task_output.actions_executed:
        status = "✓" if action.get("success") else "✗"
        print(f"  {status} {action.get('tool')}: {action.get('result', '')[:50]}...")
```
