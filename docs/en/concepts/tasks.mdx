...
## Auditable Task Output Receipts

For safety-sensitive crews, treat a task's final answer as the agent's output, not as proof that every claim in the output is supported.

A task result such as:
...
Those claims may need evidence from tool outputs, logs, file diffs, test output, human review, or task context.

One practical pattern is to ask the task to return a small completion receipt as its structured output. The receipt can then be validated with `output_pydantic`, inspected in a `callback`, or checked by a task `guardrail` before the crew proceeds.
...
class TaskReceipt(BaseModel):
    verification_status: Literal["completed", "missing_evidence", "blocked"]
    summary: str
    claims: list[Claim]
    next_owner: str
...
verification_task = Task(
    description="Run the verification steps and return a receipt for the task output.",
    expected_output="A TaskReceipt with claims, evidence, next_owner, and human_decision_required.",
    agent=verification_agent,
    output_pydantic=TaskReceipt,
)
```

A receipt with evidence can look like this:
...
A receipt with missing evidence can look like this:
...
claims:
  - claim: "Task is complete."
    ...
    evidence: []
    required_fix: "Attach the relevant tool output, file diff, or log."
  - claim: "All tests passed."
...
The `next_owner` field should name the role or agent responsible for the next step, such as `verification_agent`, `reviewer`, or `human_approver`.
...
This pattern complements `expected_output`: the expected output describes what success should look like, while the receipt summarizes whether the final task output is actually supported by evidence.