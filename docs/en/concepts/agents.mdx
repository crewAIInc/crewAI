...
| **Code Execution Mode** _(optional)_    | `code_execution_mode`    | `Literal["safe", "microvm", "unsafe"]` | Mode for code execution: 'safe' (Docker), 'microvm' (exec-sandbox), or 'unsafe' (direct). Default is 'safe'. |
...
agent = Agent(
    ...
    code_execution_mode="safe",  # Default: "safe" (options: "safe", "microvm", "unsafe")
...
- `code_execution_mode`:
  - `"safe"`: Uses Docker with restricted-sandbox fallback (recommended for production)
  - `"microvm"`: Uses exec-sandbox (QEMU microVM isolation without Docker daemon)
...
- Use `code_execution_mode: "safe"` (Docker) or `"microvm"` (exec-sandbox) in production environments
