...
1. When you set `checkpoint` on a Crew, Flow, or Agent, CrewAI registers an event listener on the event bus
2. When a matching event fires, the listener serializes the full `RuntimeState` (entities + event record) to JSON
3. The JSON is written to the configured directory via the storage provider
4. On resume via `from_checkpoint()`, the state is deserialized and execution continues from where it left off
