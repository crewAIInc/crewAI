...
<AccordionGroup>
  ...
  <Accordion title="AI Badgr (Budget/Utility, OpenAI-compatible)">
    ...
    **Environment Variables (Two Options):**
    ...
    Option 1 - OpenAI-compatible (use with existing OpenAI env vars):
    ```toml Code
    ...
    **Supported Environment Variables:**
    - OpenAI-compatible: `OPENAI_API_KEY`, `OPENAI_API_BASE` (for seamless integration)
    - Provider-specific: `AIBADGR_API_KEY`, `AIBADGR_BASE_URL` (alternative option)
