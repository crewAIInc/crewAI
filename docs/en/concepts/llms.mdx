...
<AccordionGroup>
  ...
  <Accordion title="AI Badgr (Budget/Utility, OpenAI-compatible)">
    AI Badgr provides OpenAI-compatible API access with tier-based model naming for budget-conscious projects.
    ...
    # Optional - defaults to https://aibadgr.com/api/v1
    AIBADGR_BASE_URL=<custom-base-url>
    ```
    ...
    **Basic Usage:**
    ...
    llm = LLM(
        model="openai/premium",  # Tier-based model name
        api_key="your-api-key",  # Can also use AIBADGR_API_KEY env var
    ...
    **Advanced Configuration:**
    ...
    llm = LLM(
        ...
        base_url=os.getenv("AIBADGR_BASE_URL"),
        temperature=0.7,
        max_tokens=4000,
        timeout=60.0
    )
    ...
    **Supported Environment Variables:**
    - `AIBADGR_API_KEY`: Your AI Badgr API key (required)
    - `AIBADGR_BASE_URL`: Base URL for AI Badgr API (optional, defaults to https://aibadgr.com/api/v1)

    **Supported Models (Tier-based naming):**

    | Model    | Best For                                      |
    |----------|-----------------------------------------------|
    | basic    | Simple tasks, low-cost operations             |
    | normal   | General-purpose tasks, balanced performance   |
    | premium  | Complex reasoning, high-quality outputs       |

    **Advanced Model Names:**
    
    For power users, AI Badgr's API supports specific model names that it maps internally to tiers:
    - `phi-3-mini` (mapped to basic tier by AI Badgr)
    - `mistral-7b` (mapped to normal tier by AI Badgr)
    - `llama3-8b-instruct` (mapped to premium tier by AI Badgr)
    - OpenAI model names are also accepted and mapped by AI Badgr's API

    <Info>
      AI Badgr features:
      - Budget/Utility tier provider
      - OpenAI-compatible API
      - Tier-based model naming (basic, normal, premium)
      - Cost-effective for development and testing
      - Compatible with all CrewAI LLM features
    </Info>
  </Accordion>
</AccordionGroup>
