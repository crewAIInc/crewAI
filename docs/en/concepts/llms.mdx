...
<AccordionGroup>
  ...
  <Accordion title="Avian">
    CrewAI provides native integration with [Avian](https://avian.io) through the OpenAI-compatible API.
    Avian offers access to high-performance language models with competitive pricing.

    ```toml Code
    # Required
    AVIAN_API_KEY=<your-api-key>
    ```
    ...
    **Advanced Configuration:**
    ...
    llm = LLM(
        model="avian/deepseek/deepseek-v3.2",
        api_key="your-api-key",  # Or set AVIAN_API_KEY
        temperature=0.7,
        max_tokens=4000,
        stream=True,
    )
    ...
    **Supported Environment Variables:**
    ...
    - `AVIAN_API_BASE`: Custom base URL (optional, defaults to `https://api.avian.io/v1`)

    **Features:**
    - OpenAI-compatible API (chat completions, streaming, function calling)
    - Native SDK integration (no LiteLLM dependency required)
    - Competitive pricing
