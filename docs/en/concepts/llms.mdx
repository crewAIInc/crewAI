...
<AccordionGroup>
  ...
  <Accordion title="Tzafon">
    CrewAI provides native integration with [Tzafon](https://tzafon.ai) through the OpenAI-compatible Tzafon API.
    ...
    **Basic Usage:**
    ...
    llm = LLM(
        ...
        api_key="sk_your_api_key",  # Or set TZAFON_API_KEY
        temperature=0.7
    )
    ```
    ...
    **Advanced Configuration:**
    ...
    llm = LLM(
        ...
        base_url="https://api.tzafon.ai/v1",  # Optional custom endpoint
        temperature=0.7,
        max_tokens=4000,
        stop=["END"],
        stream=True,  # Enable streaming
    ...
    **Supported Environment Variables:**
    - `TZAFON_API_KEY`: Your Tzafon API key (required)
    - `TZAFON_BASE_URL`: Custom base URL (optional, defaults to `https://api.tzafon.ai/v1`)
    ...
    **Supported Models:**
    ...
    | Model                       | Best For                                     | Pricing (Input / Output)       |
    |-----------------------------|----------------------------------------------|--------------------------------|
    | tzafon.sm-1                 | General-purpose tasks, quick responses       | $0.20 / $0.30 per 1M tokens   |
    | tzafon.northstar-cua-fast   | Browser and desktop automation agents        | $0.30 / $1.00 per 1M tokens   |

    <Info>
      Tzafon's API is fully OpenAI SDK-compatible. You can also use it directly with the OpenAI Python SDK by changing the `base_url`:
