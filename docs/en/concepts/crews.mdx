...
| **Streaming Config** _(optional)_     | `streaming_config`     | Configuration for streaming behavior including heartbeat interval and timeouts. Useful for slow local LLM models. See [Slow Local Model Support](#slow-local-model-support).                                                                                |
| **Iteration Delay** _(optional)_      | `iteration_delay`      | Seconds between iterations in continuous mode. Defaults to `1.0`.                                                                                                                                                                                            |
| **Max Continuous Runtime** _(optional)_ | `max_continuous_runtime` | Maximum runtime in seconds for continuous mode. `None` for unlimited.                                                                                                                                                                                      |
| **Health Check Interval** _(optional)_ | `health_check_interval` | Seconds between health checks in continuous mode. Defaults to `60`.                                                                                                                                                                                        |
...
Each `StreamChunk` contains metadata for UI display:

| Property | Description |
|----------|-------------|
| `content` | The text content |
| `chunk_type` | Type: TEXT, TOOL_CALL, TASK_STARTED, TASK_COMPLETED, HEARTBEAT, ERROR |
...
| `task_info` | Detailed task info for lifecycle events |
| `agent_info` | Detailed agent info for lifecycle events |

```python Code
from crewai.types import StreamChunkType

for chunk in streaming:
    if chunk.chunk_type == StreamChunkType.TASK_STARTED:
        print(f"\n[Starting: {chunk.task_name}]")
    elif chunk.chunk_type == StreamChunkType.TASK_COMPLETED:
        print(f"\n[Completed: {chunk.task_name}]")
...
#### UI Output for Dashboards

Get structured output for rendering in user interfaces:

```python Code
# During streaming - get real-time state
for chunk in streaming:
    ui_state = streaming.get_ui_state()
    print(f"Progress: {ui_state.crew.completed_tasks}/{ui_state.crew.total_tasks}")

# After streaming - get final UI output
ui_output = streaming.get_ui_output()

for task in ui_output.tasks:
    print(f"{task.name}: {task.status} ({task.duration_seconds}s)")
    if task.agent:
...
#### Slow Local Model Support

When using local LLMs (like Ollama, LM Studio, or other self-hosted models), response times can be significantly longer than cloud APIs. CrewAI provides `StreamingConfig` to handle these scenarios with heartbeat feedback and retry logic.

```python Code
from crewai import Crew
from crewai.utilities.streaming import StreamingConfig
from crewai.types import StreamChunkType

# Configure for slow local models
config = StreamingConfig(
    heartbeat_interval=30.0,  # Emit heartbeat every 30 seconds
    queue_poll_timeout=1.0,   # Poll interval for queue
    llm_timeout=None,         # Wait indefinitely for LLM response
...
for chunk in crew.kickoff(inputs={"topic": "AI"}):
    if chunk.chunk_type == StreamChunkType.HEARTBEAT:
        # Keep UI alive during long waits
        print(f"[Still working...] {chunk.content}")
    else:
        print(chunk.content, end="", flush=True)
```

| StreamingConfig Option | Default | Description |
|------------------------|---------|-------------|
| `heartbeat_interval` | `30.0` | Seconds between heartbeat emissions during LLM waits |
| `queue_poll_timeout` | `1.0` | Timeout for queue polling (enables heartbeat detection) |
| `llm_timeout` | `None` | Maximum wait time for LLM response (`None` = unlimited) |
| `max_retries` | `3` | Maximum retry attempts on connection errors |
| `retry_delay` | `1.0` | Initial delay before first retry (seconds) |
| `retry_backoff` | `2.0` | Multiplier for exponential backoff |

<Tip>
**Heartbeat Chunks**: The `HEARTBEAT` chunk type keeps your UI responsive during long LLM waits. The chunk content includes the wait duration (e.g., "Still waiting for LLM response... (45s)").
</Tip>

### Continuous Mode Execution

For 24/7 operations like trading bots, monitoring systems, or always-on automation, use continuous mode:
...
crew = Crew(
    agents=[monitor_agent, trader_agent],
...
handle = crew.continuous_kickoff(
    monitoring_directive="Monitor BTC price. Trade on 5% dips."
)

# Check status
print(f"Running: {handle.is_running}")
print(f"Iterations: {handle.iterations}")
print(f"Uptime: {handle.uptime} seconds")

# Control the running crew
handle.pause()   # Pause operation
handle.resume()  # Resume operation
handle.stop()    # Stop gracefully
```

#### Continuous Mode with Streaming

Get real-time token streaming from your continuous agents:
...
async def main():
    stream = crew.continuous_kickoff_stream(
        monitoring_directive="Monitor markets continuously"
    )
...
| Method | Type | Description |
|--------|------|-------------|
| `continuous_kickoff()` | Sync | Start continuous operation, returns handle |
| `continuous_kickoff_async()` | Async | Async version of continuous kickoff |
| `continuous_kickoff_stream()` | Async | Start with real-time streaming |
| `stop()` | Sync | Stop continuous operation |
| `pause()` | Sync | Pause continuous operation |
| `resume()` | Sync | Resume paused operation |

Learn more about continuous mode in the [Continuous Mode](/en/concepts/continuous-mode) guide.
