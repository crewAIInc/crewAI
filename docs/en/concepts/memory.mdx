...
<Warning>
For real per-customer isolation in a multi-tenant deployment, use `tenant_id`, not `scope`. Scope is a structural path; it does not stop a recall from another customer's session returning the wrong row. See [Memory Isolation](/en/concepts/memory-isolation).
</Warning>

```python
memory = Memory()

# Each customer is isolated at the storage layer via tenant_id.
memory.remember("Prefers email communication", tenant_id="acme-corp",
                scope="/preferences/communication")
memory.remember("On enterprise plan, 50 seats", tenant_id="acme-corp",
                scope="/account/plan")

# Shared product docs use the default tenant and are read by every
# customer's recall via a separate Memory or a tenant-admin view.
memory.remember("Rate limit is 1000 req/min on enterprise plan",
                scope="/product/docs")
```
