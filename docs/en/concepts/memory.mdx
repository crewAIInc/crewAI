...
- **Configurable Modes**: Choose between "summary" and "raw_messages" modes for different use cases
...
**Raw Messages Mode** (For complete conversation history):
```python
# Ensure user and thread exist first (see Basic Zep User Storage section above)
user_storage = ZepUserStorage(
    client=zep_client,
    user_id="user_123",
    thread_id="session_456",
    mode="raw_messages"  # Stores complete conversation history
)
```
