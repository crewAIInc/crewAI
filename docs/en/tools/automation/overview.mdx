...
<CardGroup cols={2}>
  ...
  <Card title="Klavis MCP Server Tools" icon="bolt" href="/en/tools/automation/klavistool">
    Easiest way to connect your AI applications to enterprise-grade MCP Servers
  </Card>
...
```python
from crewai_tools import ApifyActorTool, ComposioTool, KlavisMcpServerTools, MultiOnTool

# Create automation tools
apify_automation = ApifyActorTool()
platform_integration = ComposioTool()
klavis_mcp_server_integration = KlavisMcpServerTools()
browser_automation = MultiOnTool()

# Add to your agent
agent = Agent(
    role="Automation Specialist",
    tools=[apify_automation, platform_integration, klavis_mcp_server_integration, browser_automation],
