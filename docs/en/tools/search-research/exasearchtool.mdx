...
Get an API key at [exa.ai](https://exa.ai/) (sign up for a free account, then create a key from the dashboard).
...
- `type` (str, optional): The search type to use. Defaults to `"auto"`. Options include `"auto"`, `"neural"`, and `"keyword"`.
...
- **Neural Search**: Go beyond keywords — find results based on meaning using Exa's neural search
- **Multiple Search Modes**: Choose between `auto`, `neural`, or `keyword` search types
