...
<CardGroup cols={2}>
  ...
  <Card title="Tavily Extract Tool" icon="file-text" href="/en/tools/search-research/tavilyextractortool">
    Extract structured content from web pages using the Tavily API.
  </Card>

  <Card title="Tavily Crawl Tool" icon="spider" href="/en/tools/search-research/tavilycrawltool">
    Crawl websites and extract content from multiple pages at scale.
  </Card>
...
from crewai_tools import (
    SerperDevTool, GitHubSearchTool, YoutubeVideoSearchTool,
    TavilySearchTool, TavilyExtractTool, TavilyCrawlTool, TavilyMapTool
)
...
content_extractor = TavilyExtractTool()
site_crawler = TavilyCrawlTool()
site_mapper = TavilyMapTool()

# Add to your agent
agent = Agent(
    role="Research Analyst",
    tools=[web_search, code_search, video_research, tavily_search, content_extractor, site_crawler, site_mapper],
