...
<CardGroup cols={2}>
  ...
  <Card title="OCI Generative AI Agent Tool" icon="cloud" href="/en/tools/integration/ociinvokeagenttool">
    Call Oracle Cloud Infrastructure Generative AI agent endpoints directly from your crews.
  </Card>
</CardGroup>
...
from crewai_tools import InvokeCrewAIAutomationTool, OCIGenAIInvokeAgentTool
...
# Managed agent on OCI
oracle_router = OCIGenAIInvokeAgentTool(
...
automation_strategist = Agent(
    ...
    tools=[analysis_automation, knowledge_router, oracle_router],
    verbose=True,
)
...
- **Reuse sessions**: Bedrock and OCI agent tools support session IDs so you can maintain context across multiple tool calls
