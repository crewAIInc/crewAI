...
The following community-maintained MCP servers are compatible with CrewAI's MCP integration.
...
with MCPServerAdapter(server_params) as hiveagent_tools:
    agent = Agent(
        role="Payments Agent",
        goal="Process payments and verify agent identity",
        backstory="Agent with access to 1,221 HiveAgent tools",
...
**Usage with DSL (simple string reference):**
...
agent = Agent(
    role="Payments Agent",
    goal="Process payments and verify agent identity",
    backstory="Agent with access to HiveAgent tools",
    mcps=["https://hiveagentiq.com/mcp"],
)
```
