...
Running [AI Trace Auditor](https://github.com/BipinRimal314/ai-trace-auditor) against the CrewAI codebase reveals the full scope of external services the framework can connect to:

- **Files scanned:** 1,027
- **AI providers:** Anthropic, OpenAI, Google Generative AI, LangChain, LlamaIndex
- **Model identifiers:** 28 (claude-3-opus, claude-3-5-sonnet, gpt-4o, gemini, etc.)
...
**Legend:** Green = controller (you own the data). Blue = processor (third party, requires Data Processing Agreement).

## GDPR role classification

Each external service has a different GDPR relationship depending on how it's deployed:

| Service | Default Role | Why | Action Required |
|---------|-------------|-----|----------------|
| Anthropic API | Processor | Cloud API; Anthropic processes prompts on your behalf | DPA required (Article 28) |
| OpenAI API | Processor | Cloud API; same as above | DPA required |
| Google GenAI | Processor | Cloud API | DPA required |
| AWS Bedrock | Processor | Managed inference; AWS processes on your behalf | DPA via AWS agreement |
| GCP Vertex AI | Processor | Managed inference | DPA via GCP agreement |
| Pinecone / Qdrant Cloud | Processor | Cloud-hosted vector storage | DPA required |
| ChromaDB | Controller | Typically self-hosted; data stays on your infrastructure | No third-party DPA |
| PostgreSQL / Redis / SQLite | Controller | Self-hosted databases | No third-party DPA |
| AWS S3 | Processor | Cloud storage | DPA via AWS agreement |
...
High-risk systems need Annex IV technical documentation before market placement. Generate a starting template from your codebase:

```bash
pip install ai-trace-auditor
aitrace docs ./your-crewai-project -o annex-iv-docs.md
```

The tool auto-populates approximately 44% of the required sections from code analysis. The remaining sections require your input on intended purpose, risk assessment, and monitoring plans.
...
If you have LLM traces from your CrewAI deployment, audit them against Article 12:

```bash
aitrace audit your-traces.json -r "EU AI Act"
```

For CrewAI specifically, ensure you log:
...
Generate a complete compliance package covering Articles 11, 12, and 13 plus GDPR Article 30:

```bash
aitrace comply ./your-crewai-project --split -o compliance-package/
```

This produces:
- `article-11-docs.md` — Annex IV technical documentation template
- `article-13-flows.md` — Data flow diagram with GDPR annotations
- `compliance-summary.md` — Combined overview with coverage table
