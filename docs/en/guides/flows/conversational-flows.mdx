...
The experimental [`ConversationalFlow`](#high-level-conversationalflow-experimental) uses the same tracing lifecycle: `defer_trace_finalization` defaults to `True`, so each `handle_turn()` keeps the session trace open. Always finalize at the end of the session — either in a `finally` block around your loop, or via `ChatSession.close()`. Without one of these calls the trace batch stays open and the final conversation may never export.
