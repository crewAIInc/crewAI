...
crew = Crew(
    ...
    tasks=[task],
    manager_agent=manager, # Set the custom manager agent
...
...
# Define your agents & tasks here
...

# Instantiate your crew with a manager LLM
crew = Crew(
    agents=[researcher, writer],
    tasks=[task],
    process=Process.hierarchical,
    manager_llm=manager_llm # Set the manager LLM
)
```

## LLM Model Recommendations for Manager Agents

Not all LLM models perform equally well with delegation tasks. 
For best results with hierarchical processes and delegation, use more capable models.

Small models like `GPT-4o-mini` and smaller local models struggle with delegation to other agents. 

For example, based on our testing between `GPT-4o` and `GPT-4o-mini`:

- **Recommended**: `GPT-4o` works well for manager agents and delegation
- **Not Recommended**: `GPT-4o-mini` struggles with delegation to other agents
