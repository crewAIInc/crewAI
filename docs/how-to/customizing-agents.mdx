...
<Note>
**LLM Model Considerations for Delegation**

When enabling delegation, be aware that the performance of delegation depends on the LLM model used:

- **Recommended**: More capable models like `GPT-4o` handle delegation effectively
- **Not Recommended**: Smaller models like `GPT-4o-mini` and `smaller local models` struggle with delegation tasks

For optimal delegation results, ensure your agents are using sufficiently powerful LLM models.
</Note>
