...
2. `ConversationalFlow.builtin_route_descriptions[label]` — نص جاهز من الإطار لـ `converse` و`end` و`answer_from_history` (مصاغ لـ LLM التوجيه).
...
تستخدم [`ConversationalFlow`](#conversationalflow-عالي-المستوى-تجريبي) التجريبية نفس دورة حياة tracing: `defer_trace_finalization` افتراضياً `True`، فيبقي كل `handle_turn()` أثر الجلسة مفتوحاً. أنهِ دوماً عند نهاية الجلسة — إما داخل `finally` حول حلقتك، أو عبر `ChatSession.close()`. بدون أحد هذين، تبقى الدفعة مفتوحة وقد لا تُصدَّر آخر محادثة أبداً.
