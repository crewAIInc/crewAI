...
<AccordionGroup>
  <Accordion title="TrustedRouter">
    TrustedRouter is an OpenAI-compatible router for privacy-sensitive agent work. It uses [open-source, verifiable attested routing](https://trustedrouter.com/trust) and does not log prompts or outputs by default.

    ```toml Code
    TRUSTEDROUTER_API_KEY=tr_...
    ```
    ...
    **Basic Usage:**
    ...
    llm = LLM(
        model="openai/trustedrouter/zdr",
    ...
    Use this configuration when you want CrewAI to keep the familiar OpenAI-compatible request shape while routing through TrustedRouter's attested gateway.
  </Accordion>
