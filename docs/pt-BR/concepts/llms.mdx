...
<AccordionGroup>
  ...
  <Accordion title="Tzafon">
    O CrewAI oferece integração nativa com o [Tzafon](https://tzafon.ai) através da API compatível com OpenAI.

    ```toml Code
    # Obrigatório
    TZAFON_API_KEY=sk_your_api_key

    # Opcional
    TZAFON_BASE_URL=https://api.tzafon.ai/v1
    ```
    ...
    **Uso Básico:**
    ...
    llm = LLM(
        ...
        api_key="sk_your_api_key",  # Ou defina TZAFON_API_KEY
        temperature=0.7
    )
    ```
    ...
    **Configuração Avançada:**
    ...
    llm = LLM(
        ...
        base_url="https://api.tzafon.ai/v1",  # Endpoint personalizado opcional
        temperature=0.7,
        max_tokens=4000,
        stop=["END"],
        stream=True,  # Ativar streaming
        timeout=60.0,  # Tempo limite da requisição em segundos
        max_retries=3  # Número máximo de tentativas
    )
    ```

    **Variáveis de Ambiente Suportadas:**
    - `TZAFON_API_KEY`: Sua chave de API do Tzafon (obrigatório)
    - `TZAFON_BASE_URL`: URL base personalizada (opcional, padrão: `https://api.tzafon.ai/v1`)

    **Recursos:**
    - API de chat completions compatível com OpenAI
    - Suporte a streaming para respostas em tempo real
    - Suporte a chamadas de ferramentas / funções
    - Rastreamento de uso de tokens
    - Suporte a sequências de parada
    - Saídas estruturadas via modelos Pydantic

    **Modelos Suportados:**

    | Modelo                      | Melhor Para                                  | Preço (Entrada / Saída)        |
    |-----------------------------|----------------------------------------------|--------------------------------|
    | tzafon.sm-1                 | Tarefas de uso geral, respostas rápidas      | $0.20 / $0.30 por 1M tokens   |
    | tzafon.northstar-cua-fast   | Agentes de automação de navegador e desktop  | $0.30 / $1.00 por 1M tokens   |

    <Info>
      A API do Tzafon é totalmente compatível com o SDK da OpenAI. Você também pode usá-la diretamente com o SDK Python da OpenAI alterando o `base_url`:
