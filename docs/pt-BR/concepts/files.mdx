...
```bash
uv pip install 'crewai[file-processing]'
```
</Note>
...
# Arquivo genérico (detecta automaticamente o tipo)
file = File(source="document.pdf")  # Detectado como PDF
file = File(source="photo.jpg")     # Detectado como Imagem
```
...
# Quando você tem bytes brutos (ex: de uma resposta de API)
image_bytes = download_image_from_api()
image = ImageFile(source=FileBytes(data=image_bytes, filename="downloaded.png"))

# Ou sem nome de arquivo (será detectado automaticamente conforme necessário)
...
# Strict: Levanta erro se o arquivo exceder os limites
image = ImageFile(source="large.png", mode="strict")

# Auto: Redimensiona/divide automaticamente para caber nos limites (padrão)
image = ImageFile(source="large.png", mode="auto")

# Warn: Registra aviso mas tenta enviar mesmo assim
image = ImageFile(source="large.png", mode="warn")

# Chunk: Divide arquivos grandes em múltiplos chunks
