...
The `TavilyExtractTool` accepts the following arguments:

- `urls` (Union[List[str], str]): **Required**. A single URL string or a list of URL strings to extract data from.
- `extract_depth` (Literal["basic", "advanced"], optional): The depth of extraction. Defaults to `"basic"`.
...
extractor_tool = TavilyExtractTool(
    extract_depth='advanced',  # Use advanced extraction
