...
A [`ConversationalFlow`](#conversationalflow-de-alto-nível-experimental) experimental usa o mesmo ciclo de vida de tracing: `defer_trace_finalization` é `True` por padrão, então cada `handle_turn()` mantém o trace da sessão aberto. Sempre finalize ao fim da sessão — seja num `finally` ao redor do seu loop, seja via `ChatSession.close()`. Sem uma dessas chamadas o batch fica aberto e a última conversa pode nunca ser exportada.
