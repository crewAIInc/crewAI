...
proteção = HallucinationGuardrail(
    llm=LLM(model="gpt-4o-mini")
)

# Com contexto de referência explícito
proteção_com_contexto = HallucinationGuardrail(
...
minha_tarefa = Task(
    ...
    agent=meu_agente,
    guardrail=proteção  # Adiciona a proteção para validar a saída
)
```
...
proteção_rigorosa = HallucinationGuardrail(
...
proteção_clima = HallucinationGuardrail(
...
resultado_validacao = proteção(task_output)
