...
class ConsoleFormatter:
    ...
    def handle_agent_logs_execution(
        ...
        if isinstance(formatted_answer, AgentAction):
            ...
            formatted_json = self._format_tool_input_json(tool_input, pretty=True)
            ...
            # Create a syntax-highlighted JSON code block with improved formatting
            json_syntax = Syntax(
                formatted_json,
                "json",
                theme="monokai",
            ...
            # Create the JSON input panel with improved styling
            input_panel = Panel(
                json_syntax,
        ...
        for child in branch_to_use.children:
            if "Memory Retrieval" in str(child.label):
                for source_child in child.children:
                    sources_branch = source_child
                    if "Sources Used" in str(source_child.label):
    ...
    def _format_tool_input_json(self, tool_input: dict, pretty: bool = True) -> str:
        """Format tool input as JSON with configurable formatting options.

        Args:
            tool_input: The tool input dictionary to format
            pretty: Whether to use pretty formatting with extra spacing
        ...
        try:
            if pretty:
                ...
                formatted_json = json.dumps(
                    tool_input,
                    indent=4,  # Increased indentation for better readability
                    ensure_ascii=False,
            ...
            return f"Error formatting tool input: {e!s}"
