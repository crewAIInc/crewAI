...
class ToolExecutionMonitor:
    ...
    def start_monitoring(self, tool_name: str, tool_args: Dict[str, Any]) -> str:
        ...
        print(
            f"🔍 MONITORING: Started verification for {tool_name} (ID: {execution_id[:8]})"
        )
        return execution_id
    ...
    def finish_monitoring(
        self, execution_id: str, tool_result: Any
        ...
        evidence.execution_time = (
            evidence.timing_signature["monitoring_end"]
            - evidence.timing_signature["monitoring_start"]
        )
        ...
        execution_fingerprint = self._generate_execution_fingerprint(
        ...
        print(
            f"🔍 CERTIFICATE: Generated {authenticity_level.value} certificate for {execution_id[:8]}"
        )
        return certificate

    def _analyze_execution_authenticity(
        self, evidence: ExecutionEvidence, tool_result: Any
        ...
        if (
            evidence.subprocess_spawned
            or evidence.filesystem_changes
            or evidence.network_activity
        ):
    ...
    def _generate_execution_fingerprint(
        self, evidence: ExecutionEvidence, tool_result: Any
    ) -> str:
    ...
    def _detect_filesystem_changes(
        self, execution_id: str, evidence: ExecutionEvidence
    ):
        ...
        try:
            ...
            if test_dir.exists():
                ...
                for file_path in test_files:
                    ...
                    if (
                        hasattr(evidence.timing_signature, "get")
                        and "monitoring_start" in evidence.timing_signature
                    ):
                        if (
                            ...
                            evidence.side_effects_detected.add("filesystem_change")
                            print(
                                f"🔍 EVIDENCE: Filesystem change detected: {file_path}"
                            )
            ...
            if new_files:
                ...
                print(
                    f"🔍 EVIDENCE: {len(new_files)} new files detected in monitoring directory"
                )
...
def enable_strict_verification():
    ...
    print(
        "🔒 STRICT MODE: Tool execution verification enabled - fabricated tools will raise exceptions"
    )
...
if __name__ == "__main__":
    ...
    fake_result = (
        "File successfully created at /tmp/test.txt with content 'Hello World'"
    )
    ...
    cert2 = verify_tool_execution(
        "FileWriter", {"filename": "real_file.txt"}, real_result
    )
