...
class ReasoningPatternType(Enum):
    """Types of reasoning patterns that can be detected."""
...
class ReasoningEfficiencyEvaluator(BaseEvaluator):
    """Evaluates the reasoning efficiency of an agent based on LLM call patterns.

    This evaluator analyzes the sequence of LLM calls to detect inefficient patterns
    such as loops, verbosity, and other optimization opportunities.
    """
    ...
    def evaluate(
        ...
        """Evaluate reasoning efficiency based on LLM call patterns.
        ...
        Returns:
            EvaluationScore: Reasoning efficiency score
        """
        # Extract LLM calls from execution trace
        ...
        # Try to calculate time intervals between calls, but this may not be reliable
        time_intervals = []
        ...
        # Add timing info only if it seems reliable
        ...
        # Construct prompt for LLM evaluator
        prompt = [
...
"""}
        ...
        try:
            ...
            detected_patterns = evaluation_data.get("detected_patterns", "No specific patterns detected.")

            # Format the detailed feedback to match style of other evaluators
    ...
    def _detect_loops(self, llm_calls: List[Dict]) -> Tuple[bool, List[Dict]]:
        """Detect reasoning loops in the LLM call sequence.

        Looks for patterns where the agent is repeating similar content or actions
        without making progress.

        Args:
            llm_calls: List of LLM calls from execution trace

        Returns:
            Tuple of (loop_detected: bool, loop_details: List[Dict])
        """
        loop_details = []

        # Extract messages for similarity analysis
        messages = []
            ...
            for j in range(i + 1, len(messages) - 1):
                ...
                if similarity > 0.7:  # Arbitrary threshold
                    loop_details.append({
                        ...
                        "snippet": messages[i][:100] + "..."  # Preview of repeated content
                    })
    ...
    def _calculate_text_similarity(self, text1: str, text2: str) -> float:
        ...
        For a production implementation, consider using embedding similarity instead.
    ...
    def _analyze_reasoning_patterns(self, llm_calls: List[Dict]) -> Dict[str, Any]:
        """Analyze the overall reasoning pattern in the sequence of LLM calls.
        ...
        primary_pattern = ReasoningPatternType.EFFICIENT  # Default
    ...
    def _calculate_trend(self, values: List[float]) -> float:
        ...
        Returns:
            Trend coefficient between -1 and 1
        """
    ...
    def _calculate_loop_likelihood(self, call_lengths: List[float], response_times: List[float]) -> float:
        """Calculate the likelihood that the agent is stuck in a loop.

        Args:
            call_lengths: List of message lengths
            response_times: List of response times (may be empty or unreliable)

        Returns:
            Loop likelihood score between 0 and 1
        """
        ...
        if response_times and len(response_times) >= 3:
            try:
                ...
                if mean_time > 0:
                    ...
                    # High consistency in response times can indicate loops
                    indicators.append(max(0, time_consistency - 0.3) * 1.5)  # Scale factor
            except:
                # Just skip timing analysis if there are any issues
                pass
    ...
    def _get_call_samples(self, llm_calls: List[Dict]) -> str:
        """Extract representative samples from LLM calls for analysis.

        Args:
            llm_calls: List of LLM calls

        Returns:
            String with formatted call samples
        """
        samples = []

        # Determine which calls to sample
        if len(llm_calls) <= 6:
            # For few calls, include all
            sample_indices = list(range(len(llm_calls)))
        else:
            # For many calls, sample beginning, middle, and end
