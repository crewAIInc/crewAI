...
def test_real_vs_fake_tool_execution():
    ...
    if "total_executions" in report:
        ...
        for cert in report["certificates"]:
            print(
                f"  {cert['tool_name']}: {cert['authenticity_level']} "
...
def test_strict_mode():
    ...
    try:
        fake_result = verified_fake_tool.invoke(
            {"filename": "strict_test.txt", "content": "This should raise an exception"}
        )
        print(
            f"❌ ERROR: Fake tool succeeded when it should have failed: {fake_result}"
        )
